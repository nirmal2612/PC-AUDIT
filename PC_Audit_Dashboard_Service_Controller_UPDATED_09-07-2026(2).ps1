...
# ====================================================================================================
# Default browser login credentials:
# Username: PCAudit
# Password: PC_Audit
# The browser will show a login prompt before opening any dashboard URL.
$script:WebAuthRealm = "PC Audit Dashboard"
$script:WebUsername = "PCAudit"
$script:WebPassword = "PC_Audit"
...
function Test-WebBasicAuth {
    ...
    try {
        ...
        $encoded = $auth.Substring(6).Trim()
        $decoded = [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($encoded))
        $parts = $decoded.Split(":", 2)
        if ($parts.Count -ne 2) { return $false }

        return (($parts[0] -eq $script:WebUsername) -and ($parts[1] -eq $script:WebPassword))
    }
    catch { return $false }
}
...
function Send-WebAuthRequired {
    ...
    try {
        ...
        $Context.Response.Headers.Add("WWW-Authenticate", "Basic realm=`"$($script:WebAuthRealm)`", charset=`"UTF-8`"")
        $msg = "401 Unauthorized - Please enter valid PC Audit Dashboard credentials."
...
/* FAST EXCEL-LIKE COLUMN FILTERS */
.filter-toolbar {
    display:flex; gap:10px; align-items:center; flex-wrap:wrap;
    margin: 0 0 14px 0; padding: 10px; background:#252a32; border:1px solid #3f4652; border-radius:8px;
}
#globalSearch {
    width: 360px; max-width: 100%; padding: 9px 12px; border-radius:6px; border:1px solid #586170;
    background:#111820; color:#fff; outline:none;
}
.clearFilters {
    padding: 9px 14px; border:none; border-radius:6px; background:#566170; color:#fff; cursor:pointer; font-weight:600;
}
th { user-select:none; }
.th-wrap { display:flex; align-items:center; justify-content:space-between; gap:8px; }
.excel-filter-btn {
    min-width:22px; height:22px; border:1px solid rgba(255,255,255,0.25); border-radius:4px;
    background:#48515f; color:#fff; cursor:pointer; font-size:12px; line-height:18px;
}
.excel-filter-btn.active { background:#f39c12; color:#111; border-color:#f39c12; }
.filter-menu {
    position:absolute; z-index:99999; width:260px; max-width:90vw; background:#f7f7f7; color:#111;
    border:1px solid #999; border-radius:4px; box-shadow:0 8px 24px rgba(0,0,0,.45); padding:8px; font-size:12px;
}
.filter-menu input[type="text"] { width:100%; padding:7px 8px; margin-bottom:7px; border:1px solid #aaa; border-radius:3px; }
.filter-list { max-height:230px; overflow:auto; border:1px solid #ccc; background:#fff; padding:4px; }
.filter-item { display:block; padding:3px 2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.filter-actions { display:flex; gap:8px; justify-content:flex-end; margin-top:8px; }
.filter-actions button { padding:6px 12px; border:0; border-radius:3px; cursor:pointer; }
.filter-ok { background:#217346; color:#fff; }
.filter-cancel { background:#777; color:#fff; }
...
(function(){
    ...
    function isDateColumn(header) {
        return /date|time/i.test(header || '');
    }
    function normalizeDateOnly(v) {
        v = (v || '').trim();
        let m = v.match(/\b(\d{4}-\d{2}-\d{2})\b/);
        if (m) return m[1];
    ...
    function cellValue(row, idx) {
        const header = ths[idx] ? ths[idx].innerText.replace('🔽','').trim() : '';
        let v = (row.cells[idx] ? row.cells[idx].innerText : '').trim();
        if (isDateColumn(header)) v = normalizeDateOnly(v);
        return v || '(Blank)';
    }

    rows.forEach(r => {
        r._allText = Array.from(r.cells).map(c => c.innerText.toLowerCase()).join(' ');
        r._values = ths.map((_, i) => cellValue(r, i));
    });

    function applyFilters() {
        const kw = (globalSearch && globalSearch.value ? globalSearch.value.toLowerCase().trim() : '');
        let shown = 0;
        for (const r of rows) {
            let ok = !kw || r._allText.indexOf(kw) >= 0;
            if (ok) {
                for (const idx in activeFilters) {
                    const set = activeFilters[idx];
                    if (set && set.size && !set.has(r._values[idx])) { ok = false; break; }
                }
            }
        ...
        ths.forEach((th, i) => {
            const b = th.querySelector('.excel-filter-btn');
            if (b) b.classList.toggle('active', !!(activeFilters[i] && activeFilters[i].size));
        });
        if (filterCount) filterCount.textContent = shown + ' / ' + rows.length + ' rows';
    }
    ...
    function openMenu(idx, btn) {
        ...
        const allValues = Array.from(new Set(rows.map(r => r._values[idx]))).sort((a,b) => a.localeCompare(b, undefined, {numeric:true}));
        const selected = activeFilters[idx] ? new Set(activeFilters[idx]) : new Set(allValues);

        menu.innerHTML = `
            <input type="text" class="filter-search" placeholder="Search...">
            <label class="filter-item"><input type="checkbox" class="select-all"> Select All</label>
        ...
        menu.style.left = Math.min(rect.left + window.scrollX, window.scrollX + window.innerWidth - menu.offsetWidth - 10) + 'px';
        menu.style.top = (rect.bottom + window.scrollY + 4) + 'px';
        currentMenu = menu;

        const list = menu.querySelector('.filter-list');
        const search = menu.querySelector('.filter-search');
        const selAll = menu.querySelector('.select-all');

        function renderList(term='') {
            const frag = document.createDocumentFragment();
            const visibleVals = allValues.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) >= 0);
            for (const v of visibleVals) {
                ...
                cb.addEventListener('change', () => {
                    ...
                    selAll.checked = visibleVals.every(x => selected.has(x));
                });
            ...
            selAll.checked = visibleVals.length > 0 && visibleVals.every(x => selected.has(x));
        }
        ...
        selAll.addEventListener('change', () => {
            ...
            const visibleVals = allValues.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) >= 0);
            visibleVals.forEach(v => selAll.checked ? selected.add(v) : selected.delete(v));
            renderList(term);
        });
        menu.querySelector('.filter-ok').addEventListener('click', () => {
            if (selected.size === allValues.length) delete activeFilters[idx];
            else activeFilters[idx] = selected;
            closeMenu(); applyFilters();
        });
        menu.querySelector('.filter-cancel').addEventListener('click', closeMenu);
        setTimeout(() => search.focus(), 0);
    }

    ths.forEach((th, idx) => {
        const original = th.innerHTML;
        th.innerHTML = '<div class="th-wrap"><span>' + original + '</span><button class="excel-filter-btn" title="Filter">🔽</button></div>';
        th.querySelector('.excel-filter-btn').addEventListener('click', e => {
            e.stopPropagation();
            openMenu(idx, e.currentTarget);
        });
    });
    ...
    window.clearAllExcelFilters = function() {
        activeFilters = {};
        if (globalSearch) globalSearch.value = '';
        closeMenu(); applyFilters();
    };
    document.addEventListener('click', e => { if (currentMenu && !currentMenu.contains(e.target)) closeMenu(); });
    applyFilters();
})();
...
<div class="filter-toolbar"><input id="globalSearch" type="text" placeholder="Keyword search across all rows..."><button class="clearFilters" onclick="clearAllExcelFilters()">Clear Filters</button><span id="filterCount" class="info"></span></div>
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           if (-not (Test-WebBasicAuth -Context $context)) {
...
$script:GuiAutoLockSeconds = 300
$script:GuiAutoLockInProgress = $false

function Register-GuiActivityControl {
    param([System.Windows.Forms.Control]$Control)
    if (-not $Control) { return }
    try {
        $Control.Add_MouseMove({ $script:LastGuiActivity = Get-Date })
        $Control.Add_MouseDown({ $script:LastGuiActivity = Get-Date })
        $Control.Add_KeyDown({ $script:LastGuiActivity = Get-Date })
        foreach ($child in $Control.Controls) { Register-GuiActivityControl -Control $child }
    } catch { }
}

function Lock-GuiController {
    if ($script:GuiAutoLockInProgress) { return }
    if (-not $script:MainForm -or $script:MainForm.IsDisposed) { return }
    $script:GuiAutoLockInProgress = $true
    try {
        Add-GuiLog "Controller auto-locked after 5 minutes of inactivity. Dashboard service state is unchanged." "WARN"
...
function Apply-GuiSettings {
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebUsername)) { $script:WebUsername = [string]$script:GuiSettings.WebUsername }
    if (-not [string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebPassword)) { $script:WebPassword = [string]$script:GuiSettings.WebPassword }
}
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    $timer.Interval = 1000
    $timer.Add_Tick({
        if (-not $script:GuiAutoLockInProgress -and $form.Visible -and (((Get-Date) - $script:LastGuiActivity).TotalSeconds -ge $script:GuiAutoLockSeconds)) { Lock-GuiController; return }
    ...
    Register-GuiActivityControl -Control $form
    $script:LastGuiActivity = Get-Date
    $timer.Start()
