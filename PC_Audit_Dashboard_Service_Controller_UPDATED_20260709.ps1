...
# WEB AUTHENTICATION - browser popup login for dashboard URL access
$script:WebAuthUser = "PCAudit"
$script:WebAuthPass = "PC_Audit"
$script:WebAuthRealm = "PC Audit Dashboard"
...
function Get-DetailsPage {
   ...
   if ($isPeriodType) {
       ...
       $hdr = ($columns | ForEach-Object { New-FilterHeaderHtml $_ }) -join ""
       ...
       $detailsFilterAssets = Get-DetailsFilterAssets
...
<div class="filter-toolbar"><input id="keywordSearch" class="keyword-search" placeholder="Keyword search in current view..."><button class="clear-filters" onclick="clearAllFilters()">Clear Filters</button><span id="visibleCount" class="filter-count"></span></div>
<div class="filter-toolbar"><input id="keywordSearch" class="keyword-search" placeholder="Keyword search in current view..."><button class="clear-filters" onclick="clearAllFilters()">Clear Filters</button><span id="visibleCount" class="filter-count"></span></div>
...
function downloadCSV() {
    let csv = [];
    let rows = getVisibleTableRowsForCsv();
    rows.forEach(r => {
        let row = [];
...
"@
   ...
   elseif ($isIssueType) {
       ...
       $hdr = @("SNo", "Computer Name", "IP Address", "Network", $statusColumnName, "Auditor ID", "Audit Date") | ForEach-Object { New-FilterHeaderHtml $_ }
       $hdr = $hdr -join """
...
# ====================================================================================================
# WEB AUTHENTICATION + FAST EXCEL-LIKE FILTER HELPERS
# ====================================================================================================
function Test-WebDashboardAuth {
    ...
    try {
        ...
        $plain = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($authHeader.Substring(6).Trim()))
        return ($plain -eq "$($script:WebAuthUser):$($script:WebAuthPass)")
    } catch { return $false }
}
function Send-WebDashboardAuthRequired {
    param([System.Net.HttpListenerContext]$Context)
    try {
        $Context.Response.StatusCode = 401
        $Context.Response.Headers.Add('WWW-Authenticate', ('Basic realm="' + $script:WebAuthRealm + '"'))
        $buffer = [System.Text.Encoding]::UTF8.GetBytes("Authentication required for PC Audit Dashboard.")
...
function New-FilterHeaderHtml { param([string]$ColumnName) $safe = Get-HtmlEncoded $ColumnName; return "<th data-filter='true'><span>$safe</span><button class='filter-btn' title='Filter $safe'>▼</button></th>" }
function Get-DetailsFilterAssets {
@'
<style>
.filter-toolbar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:0 0 14px 0}.keyword-search{min-width:260px;padding:9px 12px;border-radius:6px;border:1px solid #48505d;background:#252a33;color:#fff;outline:none}.clear-filters{padding:9px 14px;background:#56616f;color:#fff;border:none;border-radius:6px;cursor:pointer}th{padding-right:34px!important}th[data-filter='true']{position:sticky;top:0;z-index:2}.filter-btn{position:absolute;right:7px;top:50%;transform:translateY(-50%);width:20px;height:20px;border:0;border-radius:4px;background:#4c5665;color:#fff;cursor:pointer;font-size:10px}.filter-btn.active{background:#27ae60}.filter-menu{position:fixed;min-width:230px;max-width:340px;max-height:390px;overflow:auto;background:#20252d;border:1px solid #4c5665;border-radius:8px;box-shadow:0 14px 35px rgba(0,0,0,.45);z-index:9999;padding:10px}.filter-menu input[type='text']{width:100%;padding:8px;margin-bottom:8px;border-radius:5px;border:1px solid #4c5665;background:#15191f;color:#fff}.filter-menu .filter-actions{display:flex;gap:6px;margin-bottom:8px}.filter-menu button{padding:6px 8px;border:0;border-radius:5px;cursor:pointer;background:#3d4654;color:#fff}.filter-menu .values{max-height:245px;overflow:auto;border-top:1px solid #3c4450;padding-top:6px}.filter-menu label{display:block;padding:4px 2px;color:#e8edf5;font-size:12px;white-space:nowrap}.filter-count{color:#9aa5b1;font-size:12px}
</style>
<script>
let activeFilters={},currentFilterMenu=null;function cellText(r,i){let c=r.cells[i];return c?c.innerText.trim():''}function isDateColumn(t){let h=t.toLowerCase();return h.indexOf('date')>=0||h.indexOf('time')>=0}function normalizeValue(v,h){v=(v||'').trim();if(isDateColumn(h)){let m=v.match(/\d{4}-\d{2}-\d{2}/);if(m)return m[0];m=v.match(/\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/);if(m)return m[0];let d=new Date(v);if(!isNaN(d.getTime()))return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')}return v||'(Blank)'}function getRows(){return Array.from(document.querySelectorAll('#t tbody tr')).filter(r=>r.cells.length>1)}function applyFilters(){let kw=(document.getElementById('keywordSearch')||{value:''}).value.toLowerCase().trim();let hs=Array.from(document.querySelectorAll('#t thead th')).map(th=>th.innerText.replace('▼','').trim());let vis=0;getRows().forEach(r=>{let show=true;Object.keys(activeFilters).forEach(i=>{if(!show)return;let v=normalizeValue(cellText(r,parseInt(i)),hs[parseInt(i)]||'');if(activeFilters[i]&&activeFilters[i].length>0&&activeFilters[i].indexOf(v)<0)show=false});if(show&&kw)show=r.innerText.toLowerCase().indexOf(kw)>=0;r.style.display=show?'':'none';if(show)vis++});document.querySelectorAll('#t thead th').forEach((th,i)=>{let b=th.querySelector('.filter-btn');if(b)b.classList.toggle('active',!!(activeFilters[i]&&activeFilters[i].length))});let c=document.getElementById('visibleCount');if(c)c.textContent=vis+' visible'}function closeFilterMenu(){if(currentFilterMenu){currentFilterMenu.remove();currentFilterMenu=null}}function esc(s){return s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}function openFilter(i,btn){closeFilterMenu();let hs=Array.from(document.querySelectorAll('#t thead th')).map(th=>th.innerText.replace('▼','').trim());let head=hs[i]||'';let vals=Array.from(new Set(getRows().map(r=>normalizeValue(cellText(r,i),head)))).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));let sel=activeFilters[i]?activeFilters[i].slice():vals.slice();let m=document.createElement('div');m.className='filter-menu';m.innerHTML='<input type="text" placeholder="Search values..." class="value-search"><div class="filter-actions"><button class="all">Select All</button><button class="none">Clear</button><button class="ok">OK</button></div><div class="filter-count"></div><div class="values"></div>';document.body.appendChild(m);let r=btn.getBoundingClientRect();m.style.left=Math.min(r.left,window.innerWidth-350)+'px';m.style.top=(r.bottom+6)+'px';currentFilterMenu=m;let box=m.querySelector('.values'),search=m.querySelector('.value-search');function render(){let q=search.value.toLowerCase();box.innerHTML='';let shown=0;vals.forEach(v=>{if(q&&v.toLowerCase().indexOf(q)<0)return;shown++;let lab=document.createElement('label');lab.innerHTML='<input type="checkbox" value=""> '+esc(v);let cb=lab.querySelector('input');cb.value=v;cb.checked=sel.indexOf(v)>=0;cb.addEventListener('change',()=>{if(cb.checked&&sel.indexOf(v)<0)sel.push(v);if(!cb.checked)sel=sel.filter(x=>x!==v)});box.appendChild(lab)});m.querySelector('.filter-count').textContent=shown+' values'}search.addEventListener('input',render);m.querySelector('.all').addEventListener('click',()=>{sel=vals.slice();render()});m.querySelector('.none').addEventListener('click',()=>{sel=[];render()});m.querySelector('.ok').addEventListener('click',()=>{if(sel.length===vals.length)delete activeFilters[i];else activeFilters[i]=sel;closeFilterMenu();applyFilters()});render()}function clearAllFilters(){activeFilters={};let k=document.getElementById('keywordSearch');if(k)k.value='';closeFilterMenu();applyFilters()}document.addEventListener('DOMContentLoaded',()=>{document.querySelectorAll('#t thead th').forEach((th,i)=>{let b=th.querySelector('.filter-btn');if(b)b.addEventListener('click',ev=>{ev.stopPropagation();openFilter(i,b)})});let k=document.getElementById('keywordSearch');if(k)k.addEventListener('input',applyFilters);document.addEventListener('click',ev=>{if(currentFilterMenu&&!currentFilterMenu.contains(ev.target)&&!ev.target.classList.contains('filter-btn'))closeFilterMenu()});applyFilters()});function getVisibleTableRowsForCsv(){let rows=[document.querySelector('#t thead tr')];getRows().forEach(r=>{if(r.style.display!=='none')rows.push(r)});return rows}
</script>
'@
}
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           if (-not (Test-WebDashboardAuth -Context $context)) {
               Write-Log -Message "Unauthorized web request from $clientIP : $path$query" -Level 'WARN'
               Send-WebDashboardAuthRequired -Context $context
               continue
           }
...
# GUI globals
$script:GuiVersion = "2026.07.09 Service Controller Auth Filters AutoLock"
$script:GuiAutoLockMinutes = 5
...
function Show-LoginWindow {
    ...
    $txtUser.Location=New-Object System.Drawing.Point(150,86); $txtUser.Size=New-Object System.Drawing.Size(200,25); $txtUser.Text="PCAudit"
    ...
    $btnLogin.Add_Click({
        if ($txtUser.Text.Trim() -eq "PCAudit" -and $txtPass.Text -eq "PC_Audit") {
            $script:LoginOk = $true
            $login.Close()
        } else {
...
function Reset-GuiAutoLockTimer { $script:GuiLastActivity = Get-Date }
function Register-GuiActivityHandlers { param([System.Windows.Forms.Control]$Control) if ($null -eq $Control) { return } try { $Control.Add_MouseMove({ Reset-GuiAutoLockTimer }); $Control.Add_MouseClick({ Reset-GuiAutoLockTimer }); $Control.Add_KeyDown({ Reset-GuiAutoLockTimer }); $Control.Add_Enter({ Reset-GuiAutoLockTimer }); foreach ($child in $Control.Controls) { Register-GuiActivityHandlers -Control $child } } catch { } }
function Invoke-GuiAutoLock { param([System.Windows.Forms.Form]$Form) if ($script:GuiAutoLockActive) { return }; $script:GuiAutoLockActive = $true; try { Add-GuiLog "Controller auto-locked after $($script:GuiAutoLockMinutes) minutes of inactivity. Server state was not changed." "WARN"; $Form.Hide(); [System.Windows.Forms.MessageBox]::Show("Controller locked due to inactivity.`n`nDashboard server is still running. Login again to manage Start / Stop controls.", "PC Audit Auto-Lock", "OK", "Information") | Out-Null; while ($true) { if (Show-LoginWindow) { break }; [System.Windows.Forms.MessageBox]::Show("Login is required to unlock the service controller.", "Locked", "OK", "Warning") | Out-Null }; Reset-GuiAutoLockTimer; $Form.Show(); $Form.WindowState = 'Normal'; $Form.Activate(); Add-GuiLog "Controller unlocked successfully." "INFO" } finally { $script:GuiAutoLockActive = $false } }
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    Register-GuiActivityHandlers -Control $form
    Reset-GuiAutoLockTimer
    ...
    $timer.Add_Tick({
        ...
        if (-not $script:GuiAutoLockActive -and $form.Visible -and (((Get-Date) - $script:GuiLastActivity).TotalMinutes -ge $script:GuiAutoLockMinutes)) { Invoke-GuiAutoLock -Form $form }
