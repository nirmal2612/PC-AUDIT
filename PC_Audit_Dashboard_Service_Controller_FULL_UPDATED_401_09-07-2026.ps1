...
function Get-DetailsPage {
   ...
   if ($isPeriodType) {
       ...
       $hdr = ($columns | ForEach-Object { "<th class='filterable'>$(Get-HtmlEncoded $_)<button class='filter-btn' type='button' title='Filter'>⌄</button></th>" }) -join ""
       ...
       foreach ($r in $records) {
           ...
           foreach ($col in $stats.OriginalColumns) {
               ...
               elseif ($r._OriginalRow -and $r._OriginalRow.$col) {
                   ...
                   if ($col -match '(?i)date|time') {
                       $dt = [datetime]::MinValue
                       if ([datetime]::TryParse($rawVal, [ref]$dt)) { $rawVal = $dt.ToString('yyyy-MM-dd') }
                   }
...
th.filterable{position:sticky;top:0;padding-right:30px;}
.filter-btn{position:absolute;right:6px;top:7px;border:1px solid #65707d;background:#252a31;color:#fff;border-radius:3px;width:20px;height:20px;cursor:pointer;line-height:15px;font-size:14px;}
.filter-btn.active{background:#27ae60;border-color:#27ae60;}
.filter-panel{position:fixed;background:#f8f9fb;color:#111;border:1px solid #7d8794;border-radius:4px;box-shadow:0 8px 22px rgba(0,0,0,.35);width:260px;max-height:360px;z-index:9999;padding:9px;font-size:12px;}
.filter-panel input{width:100%;padding:7px;margin-bottom:7px;border:1px solid #b9c0c8;border-radius:3px;}
.filter-values{max-height:210px;overflow:auto;border:1px solid #d3d7dd;background:#fff;padding:5px;}
.filter-values label{display:block;padding:3px 2px;white-space:nowrap;color:#111;}
.filter-actions{display:flex;gap:6px;margin-top:8px;justify-content:flex-end;}
.filter-actions button{padding:5px 10px;border:0;border-radius:3px;cursor:pointer;}
.filter-ok{background:#27ae60;color:#fff}.filter-clear{background:#dfe3e8;color:#111}.filter-cancel{background:#e74c3c;color:#fff}
.searchbar{width:360px;max-width:calc(100vw - 40px);padding:10px 12px;margin:0 10px 20px 0;border:1px solid #555;border-radius:6px;background:#252a31;color:#fff;}
</style>
</head>
<body>
...
<div class="info"><span id="visibleCount">$(($records).Count)</span> / $(($records).Count) records found | $colCount columns</div>
<input id="globalSearch" class="searchbar" placeholder="Search any keyword..." oninput="applyAllFilters()">
...
function cellText(row, idx){ return row.cells[idx] ? row.cells[idx].innerText.trim() : ''; }
function uniqueValues(colIdx){
    const vals = new Set();
    document.querySelectorAll('#t tbody tr').forEach(function(r){ if (r.cells.length > 1) vals.add(cellText(r,colIdx)); });
    return Array.from(vals).sort(function(a,b){ return a.localeCompare(b, undefined, {numeric:true, sensitivity:'base'}); });
}
function applyAllFilters(){
    const gs = document.getElementById('globalSearch');
    const q = gs ? gs.value.toLowerCase() : '';
    let visible = 0;
    document.querySelectorAll('#t tbody tr').forEach(function(r){
        ...
        Object.keys(activeFilters).forEach(function(k){
            if (!ok) return;
            const allowed = activeFilters[k];
            if (allowed && allowed.size > 0 && !allowed.has(cellText(r, parseInt(k)))) ok = false;
        });
        r.style.display = ok ? '' : 'none';
        if (ok && r.cells.length > 1) visible++;
    });
    const vc = document.getElementById('visibleCount'); if (vc) vc.innerText = visible;
    document.querySelectorAll('th .filter-btn').forEach(function(b,i){ b.classList.toggle('active', !!activeFilters[i] && activeFilters[i].size > 0); });
}
function closeFilterPanel(){ const old = document.getElementById('filterPanel'); if(old) old.remove(); }
function openFilterPanel(btn, colIdx){
    closeFilterPanel();
    const vals = uniqueValues(colIdx);
    const current = activeFilters[colIdx] || new Set(vals);
    const panel = document.createElement('div'); panel.id='filterPanel'; panel.className='filter-panel';
    panel.innerHTML = '<input id="filterSearch" placeholder="Search in column...">' +
      '<div><label><input type="checkbox" id="selectAllFilter"> Select All</label></div>' +
      '<div class="filter-values" id="filterValues"></div>' +
      '<div class="filter-actions"><button class="filter-clear" id="clearFilter">Clear</button><button class="filter-cancel" id="cancelFilter">Cancel</button><button class="filter-ok" id="okFilter">OK</button></div>';
    document.body.appendChild(panel);
    const rect = btn.getBoundingClientRect(); panel.style.left = Math.min(rect.left, window.innerWidth-280)+'px'; panel.style.top = (rect.bottom+4)+'px';
    const list = panel.querySelector('#filterValues');
    function renderList(term){
        term = term || '';
        list.innerHTML='';
        vals.filter(function(v){ return v.toLowerCase().includes(term.toLowerCase()); }).forEach(function(v){
            const safe = escHtml(v);
            list.insertAdjacentHTML('beforeend', '<label><input type="checkbox" class="fv" value="' + safe + '"> ' + (safe || '(blank)') + '</label>');
        });
        list.querySelectorAll('.fv').forEach(function(cb){ cb.checked = current.has(cb.value); });
        panel.querySelector('#selectAllFilter').checked = list.querySelectorAll('.fv:not(:checked)').length === 0;
    }
    renderList('');
    panel.querySelector('#filterSearch').addEventListener('input', function(e){ renderList(e.target.value); });
    panel.querySelector('#selectAllFilter').addEventListener('change', function(e){ list.querySelectorAll('.fv').forEach(function(cb){ cb.checked = e.target.checked; }); });
    panel.querySelector('#cancelFilter').onclick = closeFilterPanel;
    panel.querySelector('#clearFilter').onclick = function(){ delete activeFilters[colIdx]; closeFilterPanel(); applyAllFilters(); };
    panel.querySelector('#okFilter').onclick = function(){ const chosen = new Set(Array.from(list.querySelectorAll('.fv:checked')).map(function(x){ return x.value; })); activeFilters[colIdx] = chosen; closeFilterPanel(); applyAllFilters(); };
}
document.addEventListener('click', function(e){
    if (e.target.classList.contains('filter-btn')) { e.stopPropagation(); openFilterPanel(e.target, Array.from(e.target.closest('tr').children).indexOf(e.target.closest('th'))); return; }
    if (!e.target.closest('#filterPanel')) closeFilterPanel();
});
...
"@
   ...
   elseif ($isIssueType) {
       ...
       $hdr = "<th class='filterable'>SNo<button class='filter-btn' type='button' title='Filter'>⌄</button></th><th class='filterable'>Computer Name<button class='filter-btn' type='button' title='Filter'>⌄</button></th><th class='filterable'>IP Address<button class='filter-btn' type='button' title='Filter'>⌄</button></th><th class='filterable'>Network<button class='filter-btn' type='button' title='Filter'>⌄</button></th><th class='filterable'>$statusColumnName<button class='filter-btn' type='button' title='Filter'>⌄</button></th><th class='filterable'>Auditor ID<button class='filter-btn' type='button' title='Filter'>⌄</button></th><th class='filterable'>Audit Date<button class='filter-btn' type='button' title='Filter'>⌄</button></th>"
...
<div class="info"><span id="visibleCount">$(($records).Count)</span> / $(($records).Count) records found</div>
<input id="globalSearch" class="searchbar" placeholder="Search any keyword..." oninput="applyAllFilters()">
<button class="export" onclick="downloadCSV()">Export CSV</button>
<button class="close" onclick="window.close()">Close</button>
<div class="table-container">
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           if (-not (Test-WebBasicAuthentication -Context $context)) {
               Write-Log -Message "Unauthorized web access from $clientIP" -Level 'WARN'
...
function Initialize-GuiSettings {
    $defaults = [ordered]@{
        ...
        AutoRefreshLog = $true
        WebUsername = "PCAudit"
        WebPassword = "PC_Audit"
        WebAuthVersion = (Get-Date -Format "yyyyMMddHHmmss")
        AutoLockMinutes = 5
    }
...
function Apply-GuiSettings {
    ...
    if ([string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebPassword)) { $script:GuiSettings.WebPassword = "PC_Audit" }
    if ([string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebAuthVersion)) {
        try { $script:GuiSettings | Add-Member -NotePropertyName WebAuthVersion -NotePropertyValue (Get-Date -Format "yyyyMMddHHmmss") -Force } catch { }
    }
    if ($null -eq $script:GuiSettings.AutoLockMinutes -or [int]$script:GuiSettings.AutoLockMinutes -lt 1) { $script:GuiSettings.AutoLockMinutes = 5 }
...
function Test-WebBasicAuthentication {
    param([System.Net.HttpListenerContext]$Context)

    $expectedUser = [string]$script:GuiSettings.WebUsername
    $expectedPass = [string]$script:GuiSettings.WebPassword
    if ([string]::IsNullOrWhiteSpace($expectedUser)) { $expectedUser = "PCAudit" }
    if ([string]::IsNullOrWhiteSpace($expectedPass)) { $expectedPass = "PC_Audit" }

    $authHeader = $Context.Request.Headers["Authorization"]
    if ([string]::IsNullOrWhiteSpace($authHeader) -or -not $authHeader.StartsWith("Basic ")) { return $false }
    ...
    try {
        ...
        $u = $decoded.Substring(0, $idx)
        $pw = $decoded.Substring($idx + 1)
        return (($u -eq $expectedUser) -and ($pw -eq $expectedPass))
    } catch { return $false }
}
...
function Send-WebAuthRequired {
    ...
    $realmVersion = [string]$script:GuiSettings.WebAuthVersion
    if ([string]::IsNullOrWhiteSpace($realmVersion)) { $realmVersion = "Default" }
    $Context.Response.Headers.Add("WWW-Authenticate", ('Basic realm="PC Audit Dashboard {0}"' -f $realmVersion))
...
function Test-GuiDashboardUrl {
    ...
    try {
        ...
        if (Is-DashboardRunning) {
            ...
            try {
                $authPair = "{0}:{1}" -f ([string]$script:GuiSettings.WebUsername), ([string]$script:GuiSettings.WebPassword)
                $authToken = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($authPair))
                $response = Invoke-WebRequest -Uri $url -UseBasicParsing -TimeoutSec 5 -Headers @{ Authorization = "Basic $authToken" }
...
function Show-SettingsWindow {
    ...
    $f.Size = New-Object System.Drawing.Size(660, 610)
    ...
    New-Label "Auto Lock Minutes" 24 370 | Out-Null
    $txtAutoLock = New-Text ([string]$script:GuiSettings.AutoLockMinutes) 190 368 110
    ...
    $chkBrowser.Location=New-Object System.Drawing.Point(190,408); $chkBrowser.Size=New-Object System.Drawing.Size(260,24)
    ...
    $chkTray.Location=New-Object System.Drawing.Point(190,436); $chkTray.Size=New-Object System.Drawing.Size(300,24)
    $chkTray.ForeColor=[System.Drawing.Color]::White; $chkTray.BackColor=$f.BackColor
    $f.Controls.Add($chkTray)

    $btnSave = New-Object System.Windows.Forms.Button
    $btnSave.Text="SAVE"; $btnSave.Location=New-Object System.Drawing.Point(374,512); $btnSave.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnCancel.Text="CANCEL"; $btnCancel.Location=New-Object System.Drawing.Point(486,512); $btnCancel.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnSave.Add_Click({
        ...
        $autoLockVal = 5
        if (-not [int]::TryParse($txtAutoLock.Text.Trim(), [ref]$autoLockVal) -or $autoLockVal -lt 1 -or $autoLockVal -gt 1440) {
            [System.Windows.Forms.MessageBox]::Show("Enter valid Auto Lock Minutes between 1 and 1440.", "Settings", "OK", "Warning") | Out-Null
            return
        }
        $newWebUser = $txtWebUser.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($newWebUser)) {
            [System.Windows.Forms.MessageBox]::Show("Web username cannot be empty.", "Settings", "OK", "Warning") | Out-Null
            return
        }
        $newWebPass = $txtNewWebPass.Text
        if (-not [string]::IsNullOrWhiteSpace($newWebPass)) {
            if ($txtOldWebPass.Text -ne [string]$script:GuiSettings.WebPassword) {
                [System.Windows.Forms.MessageBox]::Show("Old web password is incorrect. Password not changed.", "Settings", "OK", "Warning") | Out-Null
                return
            }
            $script:GuiSettings.WebPassword = $newWebPass
            try { $script:GuiSettings.WebAuthVersion = (Get-Date -Format "yyyyMMddHHmmss") } catch { }
        } elseif ($newWebUser -ne [string]$script:GuiSettings.WebUsername) {
            if ($txtOldWebPass.Text -ne [string]$script:GuiSettings.WebPassword) {
                [System.Windows.Forms.MessageBox]::Show("Enter the old web password to change the web username.", "Settings", "OK", "Warning") | Out-Null
                return
            }
        }
        if ($newWebUser -ne [string]$script:GuiSettings.WebUsername) {
            try { $script:GuiSettings.WebAuthVersion = (Get-Date -Format "yyyyMMddHHmmss") } catch { }
        }
        $script:GuiSettings.WebUsername = $newWebUser
        $script:GuiSettings.AutoLockMinutes = $autoLockVal
...
function Show-LockScreen {
    ...
    $lock = New-Object System.Windows.Forms.Form
    $lock.Text = "Controller Locked"
    $lock.Size = New-Object System.Drawing.Size(420, 235)
    $lock.StartPosition = "CenterParent"
    $lock.FormBorderStyle = "FixedDialog"
    $lock.MaximizeBox = $false
    $lock.MinimizeBox = $false
    $lock.ControlBox = $false
    $lock.BackColor = [System.Drawing.Color]::FromArgb(22,25,30)
    $lock.ForeColor = [System.Drawing.Color]::White

    $lbl = New-Object System.Windows.Forms.Label
    $lbl.Text = "PC Audit Controller is locked due to inactivity."
    $lbl.Location = New-Object System.Drawing.Point(35,30)
    $lbl.Size = New-Object System.Drawing.Size(350,28)
    $lbl.TextAlign = "MiddleCenter"
    $lock.Controls.Add($lbl)

    $u = New-Object System.Windows.Forms.TextBox
    $u.Location = New-Object System.Drawing.Point(145,78); $u.Size = New-Object System.Drawing.Size(200,24); $u.Text = "HH0010520"
    $lock.Controls.Add($u)
    $ul = New-Object System.Windows.Forms.Label
    $ul.Text = "Username"; $ul.Location = New-Object System.Drawing.Point(55,80); $ul.Size = New-Object System.Drawing.Size(80,22)
    $lock.Controls.Add($ul)

    $pw = New-Object System.Windows.Forms.TextBox
    $pw.Location = New-Object System.Drawing.Point(145,113); $pw.Size = New-Object System.Drawing.Size(200,24); $pw.UseSystemPasswordChar = $true
    $lock.Controls.Add($pw)
    $pl = New-Object System.Windows.Forms.Label
    $pl.Text = "Password"; $pl.Location = New-Object System.Drawing.Point(55,115); $pl.Size = New-Object System.Drawing.Size(80,22)
    $lock.Controls.Add($pl)

    $btn = New-Object System.Windows.Forms.Button
    $btn.Text = "UNLOCK"; $btn.Location = New-Object System.Drawing.Point(145,155); $btn.Size = New-Object System.Drawing.Size(95,32)
    $btn.BackColor=[System.Drawing.Color]::FromArgb(46,204,113); $btn.ForeColor=[System.Drawing.Color]::White; $btn.FlatStyle='Flat'
    $lock.Controls.Add($btn)
    $btn.Add_Click({
        if ($u.Text.Trim() -eq "HH0010520" -and $pw.Text -eq "Foxconn-FXCN-IT") { $lock.Close() }
        else { [System.Windows.Forms.MessageBox]::Show("Invalid username or password.", "Unlock Failed", "OK", "Warning") | Out-Null; $pw.Clear(); $pw.Focus() }
    })
    $pw.Add_KeyDown({ if ($_.KeyCode -eq 'Enter') { $btn.PerformClick() } })
    $lock.ShowDialog($script:MainForm) | Out-Null
}
...
function Show-MainGui {
    ...
    $markActivity = { if (-not $script:GuiLocked) { $script:LastGuiActivity = Get-Date } }
    $form.Add_MouseMove($markActivity)
    $form.Add_MouseClick($markActivity)
    $form.Add_KeyDown($markActivity)
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    $timer.Interval = 1000
    $timer.Add_Tick({
        $lockAfter = [int]$script:GuiSettings.AutoLockMinutes
        if ($lockAfter -lt 1) { $lockAfter = 5 }
        if (-not $script:GuiLocked -and $form.Visible -and $form.WindowState -ne 'Minimized' -and ((Get-Date) - $script:LastGuiActivity).TotalMinutes -ge $lockAfter) {
