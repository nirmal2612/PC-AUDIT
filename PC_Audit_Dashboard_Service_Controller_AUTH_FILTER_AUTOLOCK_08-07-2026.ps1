...
function Send-WebAuthRequired {
   ...
   try {
      ...
      $Context.Response.Headers.Add('WWW-Authenticate','Basic realm="PC Audit Dashboard"')
...
function Show-PasswordChangeWindow {
    ...
    $ok.Add_Click({
        ...
        if ($isWeb) { $script:GuiSettings.WebUserName=$u.Text.Trim(); $script:GuiSettings.WebPassword64=(ConvertTo-Base64Text $new.Text) }
