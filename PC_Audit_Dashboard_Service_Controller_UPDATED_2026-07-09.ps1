...
# WEB BASIC AUTHENTICATION - browser popup login for dashboard URL access
$script:WebAuthUsername = "PCAudit"
$script:WebAuthPassword = "PC_Audit"
$script:WebAuthRealm = "PC Audit Dashboard"
$script:GuiLockTimeoutMinutes = 5
...
.filter-toolbar { display:flex; gap:10px; align-items:center; margin:0 0 12px 0; flex-wrap:wrap; }
.keyword-search { width:280px; max-width:100%; padding:9px 12px; border-radius:6px; border:1px solid #555; background:#222832; color:#fff; outline:none; }
.clear-filters { padding:9px 14px; background:#566573; color:#fff; border:none; border-radius:6px; cursor:pointer; }
th.filterable { padding-right:32px; position:sticky; top:0; z-index:3; }
.filter-btn { float:right; margin-left:6px; border:1px solid rgba(255,255,255,.25); background:#4b5563; color:#fff; border-radius:4px; cursor:pointer; font-size:11px; line-height:16px; width:22px; height:20px; }
.filter-btn.active { background:#f39c12; color:#111; border-color:#f39c12; }
.filter-popup { position:fixed; z-index:9999; min-width:245px; max-width:310px; max-height:390px; background:#f8f9fb; color:#111; border:1px solid #8b949e; border-radius:6px; box-shadow:0 10px 28px rgba(0,0,0,.45); padding:10px; font-size:12px; }
.filter-popup input[type="text"] { width:100%; padding:7px; border:1px solid #aaa; border-radius:4px; margin-bottom:8px; }
.filter-actions { display:flex; gap:6px; margin-top:8px; justify-content:flex-end; }
.filter-actions button { padding:6px 10px; border:1px solid #999; border-radius:4px; cursor:pointer; }
.filter-list { max-height:235px; overflow:auto; border:1px solid #ddd; background:white; padding:6px; }
.filter-item { display:block; margin:4px 0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.filter-count { color:#d0d6df; font-size:12px; margin-left:8px; }
...
<div class="filter-toolbar"><input id="keywordSearch" class="keyword-search" type="text" placeholder="Search visible table..."><button class="clear-filters" onclick="clearAllFilters()">Clear Filters</button><span id="filterCount" class="filter-count"></span></div>
...
function getCellDisplayValue(cell, headerText) {
    let txt = (cell ? cell.innerText : '').trim();
    if (/date|time/i.test(headerText)) {
        let m = txt.match(/\d{4}-\d{2}-\d{2}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/);
        if (m) { return m[0]; }
    }
    return txt || '(Blank)';
}
...
function setupExcelFilters() {
    ...
    headers.forEach((th, index) => {
        ...
        btn.innerHTML = '▼';
        btn.onclick = function(ev) { ev.stopPropagation(); openFilterPopup(index, title, btn); };
        th.appendChild(btn);
    });
...
function openFilterPopup(colIndex, title, btn) {
    closeFilterPopup();
    const table = document.getElementById('t');
    const values = new Map();
    Array.from(table.tBodies[0].rows).forEach(row => {
        if (row.cells.length <= colIndex) { return; }
        const val = getCellDisplayValue(row.cells[colIndex], title);
        values.set(val, (values.get(val) || 0) + 1);
    });
    const selected = activeFilters[colIndex] ? new Set(activeFilters[colIndex]) : new Set(values.keys());
    const popup = document.createElement('div');
    popup.className = 'filter-popup';
    let items = Array.from(values.keys()).sort((a,b) => a.localeCompare(b, undefined, {numeric:true}));
    popup.innerHTML = '<input type="text" placeholder="Search filter items..." id="filterItemSearch">' +
        '<label class="filter-item"><input type="checkbox" id="selectAllFilter"> (Select All)</label>' +
        '<div class="filter-list" id="filterList"></div>' +
        '<div class="filter-actions"><button id="clearThisFilter">Clear</button><button id="cancelFilter">Cancel</button><button id="applyFilter">OK</button></div>';
    ...
    function renderList() {
        ...
        list.innerHTML = '';
        items.filter(v => v.toLowerCase().includes(q)).forEach(v => {
            const label = document.createElement('label');
            label.className = 'filter-item';
            const cb = document.createElement('input');
            cb.type = 'checkbox'; cb.value = v; cb.checked = selected.has(v);
            cb.onchange = function() { if (cb.checked) selected.add(v); else selected.delete(v); updateSelectAll(); };
            label.appendChild(cb);
            label.appendChild(document.createTextNode(' ' + v + ' (' + values.get(v) + ')'));
    ...
    function updateSelectAll() {
        ...
        if (selectAll) { selectAll.checked = selected.size === values.size; }
    }
    document.getElementById('filterItemSearch').oninput = renderList;
    document.getElementById('selectAllFilter').onchange = function() {
        if (this.checked) { items.forEach(v => selected.add(v)); } else { items.forEach(v => selected.delete(v)); }
        renderList();
    };
    document.getElementById('clearThisFilter').onclick = function() { delete activeFilters[colIndex]; closeFilterPopup(); applyAllFilters(); };
    document.getElementById('cancelFilter').onclick = closeFilterPopup;
    document.getElementById('applyFilter').onclick = function() { activeFilters[colIndex] = Array.from(selected); closeFilterPopup(); applyAllFilters(); };
    renderList();
}

function closeFilterPopup() { if (currentPopup) { currentPopup.remove(); currentPopup = null; } }

document.addEventListener('click', function(ev) { if (currentPopup && !currentPopup.contains(ev.target)) { closeFilterPopup(); } });

function applyAllFilters() {
    const table = document.getElementById('t');
    if (!table || !table.tBodies.length) { return; }
    const headers = Array.from(table.tHead.rows[0].cells).map(th => th.getAttribute('data-title') || th.innerText.trim());
    const keyword = (document.getElementById('keywordSearch') ? document.getElementById('keywordSearch').value : '').toLowerCase();
    let visible = 0;
    Array.from(table.tBodies[0].rows).forEach(row => {
        let show = true;
        Object.keys(activeFilters).forEach(k => {
            const idx = parseInt(k, 10);
            const allowed = new Set(activeFilters[k]);
            const val = getCellDisplayValue(row.cells[idx], headers[idx]);
            if (!allowed.has(val)) { show = false; }
        });
        if (show && keyword && !row.innerText.toLowerCase().includes(keyword)) { show = false; }
        row.style.display = show ? '' : 'none';
        if (show) { visible++; }
    });
    Array.from(table.tHead.querySelectorAll('.filter-btn')).forEach((b, idx) => {
        b.classList.toggle('active', Object.prototype.hasOwnProperty.call(activeFilters, idx));
    });
    const count = document.getElementById('filterCount');
    if (count) { count.textContent = visible + ' visible record(s)'; }
}

function clearAllFilters() { activeFilters = {}; const s = document.getElementById('keywordSearch'); if (s) { s.value = ''; } applyAllFilters(); }
...
function Test-WebBasicAuthentication {
    ...
    try {
        ...
        if ($authHeader -notmatch '^Basic\s+(.+)$') { return $false }

        $encoded = $Matches[1]
        $decoded = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($encoded))
        $expected = "$($script:WebAuthUsername):$($script:WebAuthPassword)"
        return ($decoded -eq $expected)
    }
    catch {
        return $false
    }
...
function Send-WebBasicAuthenticationRequired {
    param([System.Net.HttpListenerContext]$Context)

    $Context.Response.StatusCode = 401
    $Context.Response.AddHeader('WWW-Authenticate', "Basic realm=`"$($script:WebAuthRealm)`"")
    $html = @"
<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Login Required</title></head>
<body style="font-family:Segoe UI,Arial;background:#1a1d21;color:white;padding:30px;">
<h2>PC Audit Dashboard Login Required</h2>
<p>Please enter the authorised dashboard credentials.</p>
</body>
</html>
"@
...
function Test-GuiCredential {
    param([string]$Username, [string]$Password)

    $u = if ($Username) { $Username.Trim() } else { "" }
    if ($u -eq $script:WebAuthUsername -and $Password -eq $script:WebAuthPassword) { return $true }
    # Keep previous GUI credential also valid to avoid blocking existing users.
    if ($u -eq "HH0010520" -and $Password -eq "Foxconn-FXCN-IT") { return $true }
    return $false
}
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           # WEB LOGIN: require browser authentication popup before serving any dashboard URL.
           if (-not (Test-WebBasicAuthentication -Context $context)) {
               Send-WebBasicAuthenticationRequired -Context $context
               continue
           }
...
function Show-LoginWindow {
    ...
    $txtUser.Location=New-Object System.Drawing.Point(150,86); $txtUser.Size=New-Object System.Drawing.Size(200,25); $txtUser.Text=$script:WebAuthUsername
    ...
    $btnLogin.Add_Click({
        if (Test-GuiCredential -Username $txtUser.Text -Password $txtPass.Text) {
            $script:LoginOk = $true
            $login.Close()
        } else {
...
function Show-MainGui {
    ...
    # AUTO-LOCK: lock service controller window after configured inactivity period.
    ...
    function Register-GuiActivityEvents {
        param([System.Windows.Forms.Control]$Control, [scriptblock]$Handler)
        try {
            $Control.Add_MouseMove($Handler)
            $Control.Add_MouseDown($Handler)
            $Control.Add_KeyDown($Handler)
            foreach ($child in $Control.Controls) { Register-GuiActivityEvents -Control $child -Handler $Handler }
        } catch { }
    }

    $autoLockTimer = New-Object System.Windows.Forms.Timer
    $autoLockTimer.Interval = 10000
    $autoLockTimer.Add_Tick({
        try {
            if ($script:GuiAutoLockActive) { return }
            if (((Get-Date) - $script:GuiLastActivity).TotalMinutes -ge $script:GuiLockTimeoutMinutes) {
                $script:GuiAutoLockActive = $true
                Add-GuiLog "GUI auto-locked after $($script:GuiLockTimeoutMinutes) minutes inactivity." "WARN"
                $form.Hide()
                $unlockOk = Show-LoginWindow
                if ($unlockOk) {
                    $script:GuiLastActivity = Get-Date
                    $form.Show()
                    $form.Activate()
                    Add-GuiLog "GUI unlocked successfully." "INFO"
                }
                else {
                    $form.Close()
                }
        ...
        } catch { Add-GuiLog "Auto-lock error: $_" "ERROR" }
    })
    $form.Add_Shown({
        $script:GuiLastActivity = Get-Date
        Register-GuiActivityEvents -Control $form -Handler $markGuiActivity
        $autoLockTimer.Start()
    })
    $form.Add_FormClosed({ try { $autoLockTimer.Stop(); $autoLockTimer.Dispose() } catch { } })
