...
# ====================================================================================================
# WEB BASIC AUTHENTICATION - Browser popup login before dashboard access
# ====================================================================================================
...
function Test-WebBasicAuthentication {
    ...
    try {
        ...
        Write-Log -Message "Web authentication check failed: $_" -Level 'WARN'
        return $false
    }
}

function Send-WebAuthChallenge {
    param([System.Net.HttpListenerContext]$Context)
    try {
        $Context.Response.StatusCode = 401
        $Context.Response.Headers.Add('WWW-Authenticate', ('Basic realm="{0}"' -f $script:WebAuthRealm))
        $Context.Response.ContentType = 'text/html; charset=utf-8'
        $msg = "<html><body style='font-family:Segoe UI,Arial;background:#1a1d21;color:#fff;padding:30px;'><h2>PC Audit Dashboard Login Required</h2><p>Please enter authorized credentials.</p></body></html>"
...
.searchbox { min-width:280px; padding:10px 12px; border:1px solid #4b5563; border-radius:6px; background:#111827; color:#fff; outline:none; }
.filter-summary { color:#f1c40f; font-size:13px; font-weight:600; }
th { user-select:none; z-index:5; }
.th-wrap { display:flex; align-items:center; justify-content:space-between; gap:8px; }
.filter-btn { border:0; border-radius:4px; background:#4b5563; color:#fff; cursor:pointer; padding:2px 6px; font-size:11px; line-height:16px; }
.filter-btn.active { background:#f39c12; color:#111; }
.filter-menu { position:fixed; display:none; min-width:230px; max-width:340px; max-height:360px; overflow:auto; background:#111827; color:#fff; border:1px solid #4b5563; border-radius:8px; box-shadow:0 12px 32px rgba(0,0,0,.45); padding:10px; z-index:99999; }
...
.filter-actions button { flex:1; padding:6px; border:0; border-radius:5px; cursor:pointer; background:#374151; color:#fff; }
.filter-values label { display:block; padding:4px 2px; font-size:12px; cursor:pointer; }
...
<div class="toolbar">
<input id="globalSearch" class="searchbox" type="text" placeholder="Keyword filter all columns..." oninput="applyFilters()">
<button class="export" onclick="downloadCSV()">Export Filtered CSV</button>
...
function normalizeFilterValue(text, header) {
    let value = (text || '').trim();
    if (/date|time/i.test(header || '')) {
        let m = value.match(/\b(\d{4}-\d{2}-\d{2}|\d{2}[-\/]\d{2}[-\/]\d{4}|\d{1,2}[-\/]\d{1,2}[-\/]\d{4})\b/);
        if (m) return m[1];
...
function getHeaderText(th) { return (th.getAttribute('data-title') || th.innerText || '').replace('🔽','').trim(); }
function setupExcelFilters() {
    ...
    Array.from(table.tHead.rows[0].cells).forEach((th, idx) => {
        ...
        th.innerHTML = '<div class="th-wrap"><span>' + title + '</span><button class="filter-btn" title="Filter">🔽</button></div>';
        th.querySelector('.filter-btn').addEventListener('click', function(e){ e.stopPropagation(); openFilterMenu(idx, th); });
    });
    document.addEventListener('click', function(e){ const menu = document.getElementById('filterMenu'); if (menu && !menu.contains(e.target) && !e.target.classList.contains('filter-btn')) menu.style.display='none'; });
    applyFilters();
}
function openFilterMenu(colIndex, th) {
    currentFilterColumn = colIndex;
    let menu = document.getElementById('filterMenu');
    if (!menu) {
        menu = document.createElement('div'); menu.id = 'filterMenu'; menu.className = 'filter-menu';
        menu.innerHTML = '<input id="filterValueSearch" type="text" placeholder="Search values..." oninput="renderFilterValues()"><div class="filter-actions"><button onclick="selectAllFilterValues(true)">Select All</button><button onclick="selectAllFilterValues(false)">Clear</button><button onclick="applyFilterMenu()">OK</button></div><div id="filterValues" class="filter-values"></div>';
        document.body.appendChild(menu);
    }
    const rect = th.getBoundingClientRect();
    menu.style.left = Math.min(rect.left, window.innerWidth - 360) + 'px'; menu.style.top = (rect.bottom + 4) + 'px'; menu.style.display = 'block';
    const search = document.getElementById('filterValueSearch'); if (search) search.value = '';
    renderFilterValues();
}
function getUniqueValues(colIndex) {
    const table = document.getElementById('t'); const header = getHeaderText(table.tHead.rows[0].cells[colIndex]); const set = new Set();
    Array.from(table.tBodies[0].rows).forEach(r => { if (r.cells[colIndex]) set.add(normalizeFilterValue(r.cells[colIndex].innerText, header)); });
    return Array.from(set).sort((a,b)=>a.localeCompare(b, undefined, {numeric:true, sensitivity:'base'}));
}
function renderFilterValues() {
    const list = document.getElementById('filterValues'); const search = (document.getElementById('filterValueSearch')?.value || '').toLowerCase();
    const values = getUniqueValues(currentFilterColumn).filter(v => v.toLowerCase().includes(search)); const selected = activeFilters[currentFilterColumn] || null;
    list.innerHTML = values.map(v => { const safe = v.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); const checked = !selected || selected.has(v) ? 'checked' : ''; return '<label><input type="checkbox" value="' + safe + '" ' + checked + '> ' + (safe || '(blank)') + '</label>'; }).join('') || '<div style="color:#aaa;padding:8px;">No values</div>';
}
function selectAllFilterValues(on) { document.querySelectorAll('#filterValues input[type=checkbox]').forEach(cb => cb.checked = on); }
function applyFilterMenu() {
    const checked = Array.from(document.querySelectorAll('#filterValues input[type=checkbox]:checked')).map(cb => cb.value); const all = getUniqueValues(currentFilterColumn);
    if (checked.length === all.length) delete activeFilters[currentFilterColumn]; else activeFilters[currentFilterColumn] = new Set(checked);
    document.getElementById('filterMenu').style.display = 'none'; applyFilters();
}
function applyFilters() {
    const table = document.getElementById('t'); const search = (document.getElementById('globalSearch')?.value || '').toLowerCase(); const headers = Array.from(table.tHead.rows[0].cells).map(getHeaderText);
    let visible = 0, total = 0;
    Array.from(table.tBodies[0].rows).forEach(r => { if (r.cells.length <= 1) return; total++; let show = true; const rowText = r.innerText.toLowerCase(); if (search && !rowText.includes(search)) show = false; Object.keys(activeFilters).forEach(k => { const idx = parseInt(k,10); const val = normalizeFilterValue(r.cells[idx]?.innerText || '', headers[idx]); if (!activeFilters[k].has(val)) show = false; }); r.style.display = show ? '' : 'none'; if (show) visible++; });
    Array.from(table.tHead.rows[0].cells).forEach((th, idx) => { const btn = th.querySelector('.filter-btn'); if (btn) btn.classList.toggle('active', !!activeFilters[idx]); });
    const fs = document.getElementById('filterSummary'); if (fs) fs.innerText = 'Showing ' + visible + ' of ' + total + ' rows';
}
function downloadCSV() {
    let csv = []; let table = document.getElementById('t'); let headerCells = Array.from(table.tHead.rows[0].cells); csv.push(headerCells.map(th => '"' + getHeaderText(th).replace(/"/g,'""') + '"').join(','));
    let rows = Array.from(table.tBodies[0].rows).filter(r => r.style.display !== 'none' && r.cells.length > 1);
    rows.forEach(r => { let row = []; r.querySelectorAll('td').forEach(c => { let txt = c.innerText.replace(/"/g, '""'); row.push('"' + txt + '"'); }); csv.push(row.join(',')); });
    let blob = new Blob(['\ufeff' + csv.join('\n')], {type: 'text/csv;charset=utf-8;'}); let a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = '$title-filtered-$(Get-Date -Format 'yyyyMMdd-HHmmss').csv'; a.click();
}
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           # WEB LOGIN POPUP: Require username/password before serving dashboard pages.
           if (-not (Test-WebBasicAuthentication -Context $context)) {
               Send-WebAuthChallenge -Context $context
               continue
           }
...
function Reset-GuiInactivity {
    if (-not $script:GuiLocked) { $script:GuiLastActivity = Get-Date }
}
...
function Register-GuiActivityHandlers {
    ...
    try {
        $Control.Add_MouseMove({ Reset-GuiInactivity })
        $Control.Add_MouseDown({ Reset-GuiInactivity })
        $Control.Add_KeyDown({ Reset-GuiInactivity })
        $Control.Add_Click({ Reset-GuiInactivity })
        foreach ($child in $Control.Controls) { Register-GuiActivityHandlers -Control $child }
    } catch { }
}

function Lock-GuiControllerForInactivity {
    if ($script:GuiLocked) { return }
    if (-not $script:MainForm) { return }
    $script:GuiLocked = $true
    try {
        Add-GuiLog "Controller auto-locked after $($script:GuiAutoLockMinutes) minutes of inactivity." "WARN"
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    $timer.Add_Tick({
        ...
        if (-not $script:GuiLocked -and ((Get-Date) - $script:GuiLastActivity).TotalMinutes -ge $script:GuiAutoLockMinutes) {
            Lock-GuiControllerForInactivity
            return
        }
        Update-ButtonState
