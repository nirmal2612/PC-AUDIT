...
# WEB LOGIN / BASIC AUTHENTICATION
...
function Test-WebDashboardAuth {
    ...
    try {
        ...
        return ($u -eq $script:WebAuthUsername -and $pw -eq $script:WebAuthPassword)
    } catch { return $false }
}
...
function Send-WebAuthRequired {
    ...
    try {
        ...
        $msg = '<html><body style="font-family:Segoe UI;background:#1a1d21;color:white;padding:30px;"><h2>401 Unauthorized</h2><p>Valid PC Audit Dashboard credential is required.</p></body></html>'
...
function Get-MainPage {
   ...
   # 📅 Monthly audit count history shown under Top 5 Auditors.
   $monthlyRows = ""
   $monthlyGroups = @($stats.Data | Where-Object { $_.Date } | Group-Object { $_.Date.ToString('yyyy-MM') } | Sort-Object Name -Descending)
   foreach ($mg in $monthlyGroups) {
       $monthDate = [DateTime]::ParseExact($mg.Name + '-01','yyyy-MM-dd',$null)
       $monthLabel = $monthDate.ToString('MMM yyyy')
       $monthKey = $monthDate.ToString('yyyy-MM')
       $monthlyRows += "<tr class='month-row' onclick=`"openDetail('Month_$monthKey')`"><td>$monthLabel</td><td><span class='month-count'>$($mg.Count)</span><button class='view-month' onclick=`"event.stopPropagation(); openDetail('Month_$monthKey')`">&gt;</button></td></tr>`n"
   }
   if ($monthlyRows -eq "") { $monthlyRows = "<tr><td colspan='2' style='color:#888;'>No monthly audit data</td></tr>" }
...
.monthly-history-card h3 { font-size: 1.02em; margin-bottom: 10px; color: #dfe8f5; }
.monthly-scroll { max-height: 145px; overflow-y: auto; padding-right: 4px; }
.month-row { cursor: pointer; }
.month-row:hover { background: #404754; }
.month-count { font-weight: 700; font-size: 17px; margin-right: 10px; }
.view-month {
    width: 24px; height: 24px; border: 0; border-radius: 50%; cursor: pointer;
    background: #3498db; color: white; font-size: 20px; line-height: 20px; font-weight: bold;
}
...
<div class="content-grid">
    ...
    <div class="section">
        ...
        <div class="monthly-history-card">
            <h3>📅 Monthly PC Audit Count</h3>
            <div class="monthly-scroll">
                <table>
                    <thead><tr><th>Month</th><th>Audit Count</th></tr></thead>
...
function Get-DetailsPage {
   ...
   $isMonthHistoryType = ($Type -match '^Month_(\d{4}-\d{2})$')
   ...
   if ($isPeriodType -or $isMonthHistoryType) {
       ...
       if ($isMonthHistoryType) { $baseType = 'MonthHistory' }
       switch ($baseType) {
           'Daily' {
           ...
           'MonthHistory' {
               $monthKey = [regex]::Match($Type, '^Month_(\d{4}-\d{2})$').Groups[1].Value
               $selectedMonth = [DateTime]::ParseExact($monthKey + '-01','yyyy-MM-dd',$null)
               $nextMonth = $selectedMonth.AddMonths(1)
               $monthLabel = $selectedMonth.ToString('MMM yyyy')
               $records = @($stats.Data | Where-Object {
                   $_.Date -and $_.Date -ge $selectedMonth -and $_.Date -lt $nextMonth -and
                   (-not $segmentFilter -or $_.NetworkSegment -eq $segmentFilter)
               })
               $title = "Monthly Audit Details - $monthLabel - $segmentInfo"
               $dateRangeInfo = "Month: $monthLabel | Filter: $segmentInfo"
           }
           'Total' {
               $records = @($stats.Data | Where-Object { 
                   -not $segmentFilter -or $_.NetworkSegment -eq $segmentFilter
...
.toolbar-filter { display:flex; gap:10px; align-items:center; margin: 8px 0 15px 0; }
.keyword { width: 320px; padding:10px 12px; border-radius:6px; border:1px solid #4b5563; background:#111827; color:#fff; }
.clearFilters { padding:10px 16px; border:0; border-radius:6px; background:#6b7280; color:#fff; cursor:pointer; }
th { user-select:none; }
.filter-btn { margin-left:6px; padding:1px 5px; border:1px solid #6b7280; border-radius:3px; background:#111827; color:#fff; cursor:pointer; font-size:11px; }
.filter-btn.active { background:#f39c12; color:#111; border-color:#f39c12; }
.filter-menu { position:fixed; z-index:99999; width:260px; max-height:355px; background:#111827; border:1px solid #4b5563; border-radius:8px; box-shadow:0 8px 25px rgba(0,0,0,.55); padding:10px; display:none; }
.filter-menu input[type=text] { width:100%; padding:8px; margin-bottom:8px; background:#1f2937; color:#fff; border:1px solid #4b5563; border-radius:5px; }
.filter-actions { display:flex; gap:6px; margin:8px 0; }
.filter-actions button { flex:1; padding:7px; border:0; border-radius:5px; cursor:pointer; font-weight:600; }
.filter-values { max-height:190px; overflow:auto; border:1px solid #374151; padding:6px; border-radius:5px; }
.filter-values label { display:block; padding:4px; color:#e5e7eb; cursor:pointer; }
.filter-values label:hover { background:#1f2937; }
...
<div class="toolbar-filter"><input id="keywordSearch" class="keyword" placeholder="Keyword search..." oninput="applyAllFilters()"><button class="clearFilters" onclick="clearAllFilters()">Clear Filters</button><span id="filterCount" class="info"></span></div>
<div id="excelFilterMenu" class="filter-menu"></div>
...
function normalizeFilterValue(txt, header){
    if (/date|time/i.test(header)) {
        let m = txt.match(/\d{4}-\d{2}-\d{2}/);
        if (m) return m[0];
        let d = new Date(txt); if (!isNaN(d.getTime())) return d.toISOString().slice(0,10);
    }
    return txt || '(Blanks)';
}
function setupExcelFilters(){
    const table=document.getElementById('t'); if(!table || !table.tHead) return;
    Array.from(table.tHead.rows[0].cells).forEach((th,i)=>{
        const btn=document.createElement('button'); btn.className='filter-btn'; btn.innerText='▼'; btn.title='Filter';
        btn.onclick=function(e){e.stopPropagation(); openFilterMenu(i,btn);}; th.appendChild(btn);
    });
    document.addEventListener('click',()=>{ const m=document.getElementById('excelFilterMenu'); if(m) m.style.display='none'; });
}
function openFilterMenu(col, btn){
    activeFilterCol=col; const table=document.getElementById('t'); const header=table.tHead.rows[0].cells[col].childNodes[0].textContent.trim();
    let set=new Set(); Array.from(table.tBodies[0].rows).forEach(r=>set.add(normalizeFilterValue(cellText(r,col), header)));
    let vals=Array.from(set).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
    const selected=columnFilters[col] || new Set(vals);
    const menu=document.getElementById('excelFilterMenu');
    menu.innerHTML='<input type="text" id="filterSearchBox" placeholder="Search..." oninput="filterMenuSearch()">'+
      '<label><input type="checkbox" id="selectAllFilter" onchange="toggleSelectAllFilter(this.checked)"> Select All</label>'+
      '<div class="filter-values" id="filterValues"></div><div class="filter-actions"><button onclick="applyMenuFilter()">OK</button><button onclick="clearColumnFilter()">Clear</button></div>';
    const box=menu.querySelector('#filterValues');
    vals.forEach(v=>{ const id='fv_'+Math.random().toString(36).slice(2); const checked=selected.has(v)?'checked':''; box.insertAdjacentHTML('beforeend','<label data-val="'+v.toLowerCase().replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'"><input type="checkbox" value="'+v.replace(/&/g,'&amp;').replace(/"/g,'&quot;')+'" '+checked+'> '+v+'</label>'); });
    menu.querySelector('#selectAllFilter').checked = vals.length && vals.every(v=>selected.has(v));
    const r=btn.getBoundingClientRect(); menu.style.left=Math.min(r.left, window.innerWidth-280)+'px'; menu.style.top=(r.bottom+4)+'px'; menu.style.display='block';
}
function filterMenuSearch(){ const q=document.getElementById('filterSearchBox').value.toLowerCase(); document.querySelectorAll('#filterValues label').forEach(l=>{l.style.display=l.dataset.val.includes(q)?'block':'none';}); }
function toggleSelectAllFilter(chk){ document.querySelectorAll('#filterValues label').forEach(l=>{ if(l.style.display!=='none') l.querySelector('input').checked=chk; }); }
function applyMenuFilter(){ const vals=Array.from(document.querySelectorAll('#filterValues input:checked')).map(x=>x.value); columnFilters[activeFilterCol]=new Set(vals); document.getElementById('excelFilterMenu').style.display='none'; applyAllFilters(); }
function clearColumnFilter(){ delete columnFilters[activeFilterCol]; document.getElementById('excelFilterMenu').style.display='none'; applyAllFilters(); }
function applyAllFilters(){
    const table=document.getElementById('t'); const q=(document.getElementById('keywordSearch')?.value||'').toLowerCase(); let shown=0,total=0;
    Array.from(table.tBodies[0].rows).forEach(r=>{ total++; let ok=true; const rowText=r.innerText.toLowerCase(); if(q && !rowText.includes(q)) ok=false;
        Object.keys(columnFilters).forEach(k=>{ const col=parseInt(k); const header=table.tHead.rows[0].cells[col].childNodes[0].textContent.trim(); const val=normalizeFilterValue(cellText(r,col), header); if(!columnFilters[k].has(val)) ok=false; });
        r.style.display=ok?'':'none'; if(ok) shown++;
    });
    Array.from(table.tHead.rows[0].querySelectorAll('.filter-btn')).forEach((b,i)=>{ b.classList.toggle('active', !!columnFilters[i]); });
    const fc=document.getElementById('filterCount'); if(fc) fc.innerText='Showing '+shown+' / '+total;
}
function clearAllFilters(){ columnFilters={}; const k=document.getElementById('keywordSearch'); if(k) k.value=''; applyAllFilters(); }
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           if (-not (Test-WebDashboardAuth -Context $context)) {
               Write-Log -Message "Unauthorized web request from $clientIP" -Level 'WARN'
...
function Process-GuiDashboardRequests {
    ...
    try {
        ...
        if (-not (Test-WebDashboardAuth -Context $context)) {
            Add-GuiLog "Unauthorized web request from $clientIP" "WARN"
            Send-WebAuthRequired -Context $context
            Begin-GuiDashboardAccept
            return
        }
...
function Register-GuiActivityHandlers {
    ...
    foreach ($child in $Control.Controls) { Register-GuiActivityHandlers -Control $child }
}

function Invoke-GuiAutoLock {
    if ($script:GuiLockActive) { return }
    $script:GuiLockActive = $true
    try {
        Add-GuiLog "Controller auto-locked after $($script:GuiAutoLockMinutes) minutes inactivity." "WARN"
        if ($script:MainForm) { $script:MainForm.Enabled = $false }
        $ok = Show-LoginWindow
        if ($script:MainForm) { $script:MainForm.Enabled = $true; $script:MainForm.Activate() }
        if (-not $ok) {
            Add-GuiLog "Auto-lock login cancelled. Closing controller for safety." "WARN"
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    $timer.Interval = 1000
    $timer.Add_Tick({
        if (-not $script:GuiLockActive -and ((Get-Date) - $script:LastGuiActivity).TotalMinutes -ge $script:GuiAutoLockMinutes) { Invoke-GuiAutoLock; return }
