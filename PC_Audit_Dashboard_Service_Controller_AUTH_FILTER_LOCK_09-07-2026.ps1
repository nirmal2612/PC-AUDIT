...
.search-panel { display:flex; align-items:center; gap:10px; margin:0 0 12px 0; flex-wrap:wrap; }
.search-panel input { width:320px; padding:9px 11px; border:1px solid #4b5563; border-radius:6px; background:#1f242c; color:#fff; }
.search-panel button { padding:9px 13px; border:none; border-radius:6px; background:#58606d; color:#fff; cursor:pointer; }
th { position: sticky; top: 0; z-index: 2; }
th .filter-head { display:flex; align-items:center; justify-content:space-between; gap:6px; }
.filter-btn { border:none; background:rgba(255,255,255,.13); color:#fff; border-radius:4px; width:22px; height:22px; cursor:pointer; line-height:20px; font-size:12px; }
.filter-btn.active { background:#f39c12; color:#111; }
.filter-menu { position:fixed; min-width:230px; max-width:330px; max-height:360px; overflow:auto; background:#20252d; color:#fff; border:1px solid #58606d; border-radius:8px; box-shadow:0 12px 32px rgba(0,0,0,.55); padding:10px; z-index:99999; }
.filter-menu input[type=text] { width:100%; padding:7px; margin-bottom:8px; background:#111827; color:#fff; border:1px solid #4b5563; border-radius:5px; }
.filter-options { max-height:215px; overflow:auto; border-top:1px solid #3d4550; border-bottom:1px solid #3d4550; padding:6px 0; margin:6px 0; }
.filter-option { display:block; padding:4px 2px; font-size:12px; white-space:nowrap; }
.filter-actions { display:flex; gap:6px; justify-content:flex-end; margin-top:8px; }
.filter-actions button { padding:6px 9px; border:none; border-radius:5px; cursor:pointer; }
.filter-apply { background:#27ae60; color:#fff; }
.filter-clear { background:#e67e22; color:#111; }
.filter-close { background:#6b7280; color:#fff; }
...
<div class="search-panel"><input id="globalSearch" type="text" placeholder="Keyword search..."><button onclick="clearAllFilters()">Clear Filters</button><span id="filterCountInfo"></span></div>
...
const filterState = {};
let activeMenu = null;
function normalizeCellValue(text, header) {
    text = (text || '').trim();
...
function closeFilterMenu(){ if(activeMenu){ activeMenu.remove(); activeMenu=null; } }
function getRows(){ return Array.from(tbody ? tbody.rows : []); }
function applyFilters(){
    const keyword = (document.getElementById('globalSearch')?.value || '').toLowerCase();
    let visible = 0;
    getRows().forEach(row => {
        const rowText = row.innerText.toLowerCase();
        let show = !keyword || rowText.includes(keyword);
        Object.keys(filterState).forEach(ci => {
            if(!show) return;
            const th = table.tHead.rows[0].cells[ci];
            const header = th ? th.innerText : '';
            const val = normalizeCellValue(row.cells[ci] ? row.cells[ci].innerText : '', header);
            if(filterState[ci] && filterState[ci].size > 0 && !filterState[ci].has(val)) show = false;
        });
        row.style.display = show ? '' : 'none';
        if(show) visible++;
    });
    Array.from(table.tHead.rows[0].cells).forEach((th,i)=>{
        const btn=th.querySelector('.filter-btn'); if(btn) btn.classList.toggle('active', filterState[i] && filterState[i].size>0);
    });
    const info=document.getElementById('filterCountInfo');
    if(info) info.textContent = visible + ' / ' + getRows().length + ' rows';
}
function openColumnFilter(index, btn){
    closeFilterMenu();
    const th = table.tHead.rows[0].cells[index];
    const header = th ? th.innerText : '';
    const values = Array.from(new Set(getRows().map(r => normalizeCellValue(r.cells[index] ? r.cells[index].innerText : '', header)))).sort((a,b)=>a.localeCompare(b, undefined, {numeric:true}));
    const selected = filterState[index] ? new Set(filterState[index]) : new Set(values);
    const menu=document.createElement('div'); menu.className='filter-menu';
    const rect=btn.getBoundingClientRect(); menu.style.left=Math.min(rect.left, window.innerWidth-350)+'px'; menu.style.top=(rect.bottom+4)+'px';
    menu.innerHTML = `<input type="text" placeholder="Search filter values..."><label class="filter-option"><input type="checkbox" class="select-all"> Select All</label><div class="filter-options"></div><div class="filter-actions"><button class="filter-clear">Clear</button><button class="filter-close">Close</button><button class="filter-apply">Apply</button></div>`;
    document.body.appendChild(menu); activeMenu=menu;
    const list=menu.querySelector('.filter-options'), search=menu.querySelector('input[type=text]'), selAll=menu.querySelector('.select-all');
    function render(){
        const q=search.value.toLowerCase(); list.innerHTML='';
        values.filter(v => !q || v.toLowerCase().includes(q)).forEach(v=>{
            const lab=document.createElement('label'); lab.className='filter-option';
            const cb=document.createElement('input'); cb.type='checkbox'; cb.value=v; cb.checked=selected.has(v); cb.onchange=()=>{ cb.checked?selected.add(v):selected.delete(v); syncAll(); };
            lab.appendChild(cb); lab.appendChild(document.createTextNode(' '+(v || '(blank)'))); list.appendChild(lab);
        }); syncAll();
    }
    function syncAll(){ selAll.checked = values.length>0 && values.every(v=>selected.has(v)); }
    selAll.onchange=()=>{ if(selAll.checked){ values.forEach(v=>selected.add(v)); } else { values.forEach(v=>selected.delete(v)); } render(); };
    search.oninput=render;
    menu.querySelector('.filter-apply').onclick=()=>{ filterState[index]=selected.size===values.length?new Set():selected; closeFilterMenu(); applyFilters(); };
    menu.querySelector('.filter-clear').onclick=()=>{ filterState[index]=new Set(); closeFilterMenu(); applyFilters(); };
    menu.querySelector('.filter-close').onclick=closeFilterMenu;
    render();
}
function setupExcelFilters(){
    if(!table || !table.tHead) return;
    Array.from(table.tHead.rows[0].cells).forEach((th,i)=>{
        const text=th.textContent.trim(); th.textContent='';
        const wrap=document.createElement('div'); wrap.className='filter-head';
        const span=document.createElement('span'); span.textContent=text;
        const btn=document.createElement('button'); btn.className='filter-btn'; btn.type='button'; btn.textContent='▼'; btn.title='Filter '+text;
        btn.onclick=(e)=>{ e.stopPropagation(); openColumnFilter(i, btn); };
        wrap.appendChild(span); wrap.appendChild(btn); th.appendChild(wrap);
    });
    const gs=document.getElementById('globalSearch'); if(gs) gs.addEventListener('input', applyFilters);
    document.addEventListener('click', e=>{ if(activeMenu && !activeMenu.contains(e.target) && !e.target.classList.contains('filter-btn')) closeFilterMenu(); });
    applyFilters();
}
function clearAllFilters(){ Object.keys(filterState).forEach(k=>delete filterState[k]); const gs=document.getElementById('globalSearch'); if(gs) gs.value=''; closeFilterMenu(); applyFilters(); }
setupExcelFilters();
</script>
</body>
</html>
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           if (-not (Test-WebRequestAuth -Context $context)) {
               Write-Log -Message "Unauthorized web request from $clientIP : $path$query" -Level 'WARN'
               Send-WebUnauthorized -Context $context
               continue
           }
...
function Initialize-GuiSettings {
    $defaults = [ordered]@{
        ...
        AutoRefreshLog = $true
        WebUsername = "PCAudit"
        WebPasswordHash = "759D76B0DEEC9AACC969C102B0F733161311725313C0E63A706EC148CFD77E6A"
        WebCredentialVersion = 1
    }
    ...
    if ([string]::IsNullOrWhiteSpace([string]$defaults.WebUsername)) { $defaults.WebUsername = "PCAudit" }
    if ([string]::IsNullOrWhiteSpace([string]$defaults.WebPasswordHash)) { $defaults.WebPasswordHash = "759D76B0DEEC9AACC969C102B0F733161311725313C0E63A706EC148CFD77E6A" }
    if (-not $defaults.WebCredentialVersion) { $defaults.WebCredentialVersion = 1 }
...
function Get-Sha256Text {
    ...
    try {
        ...
        return (($sha.ComputeHash($bytes) | ForEach-Object { $_.ToString('x2') }) -join '').ToUpper()
    } finally { $sha.Dispose() }
}

function Get-WebAuthRealm {
    $ver = 1
    try { $ver = [int]$script:GuiSettings.WebCredentialVersion } catch { $ver = 1 }
    return "PC Audit Dashboard v$ver"
}

function Send-WebUnauthorized {
    param($Context)
    try {
        $Context.Response.StatusCode = 401
        $Context.Response.StatusDescription = "Unauthorized"
        $Context.Response.Headers.Add("WWW-Authenticate", ('Basic realm="{0}"' -f (Get-WebAuthRealm)))
        $Context.Response.Headers.Add("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0")
        $html = "<!DOCTYPE html><html><head><title>401 Unauthorized</title></head><body style='font-family:Segoe UI,Arial;background:#1a1d21;color:white;padding:30px;'><h2>401 Unauthorized</h2><p>New or valid PC Audit dashboard credential required.</p></body></html>"
...
function Test-WebRequestAuth {
    ...
    try {
        ...
        $pw = $raw.Substring($idx + 1)
        $expectedUser = [string]$script:GuiSettings.WebUsername
        $expectedHash = [string]$script:GuiSettings.WebPasswordHash
        if ([string]::IsNullOrWhiteSpace($expectedUser)) { $expectedUser = "PCAudit" }
        if ([string]::IsNullOrWhiteSpace($expectedHash)) { $expectedHash = Get-Sha256Text "PC_Audit" }
        return (($u -eq $expectedUser) -and ((Get-Sha256Text $pw) -eq $expectedHash))
    } catch { return $false }
}
...
function Show-SettingsWindow {
    ...
    New-Label "Web Username" 24 324 | Out-Null
    $txtWebUser = New-Text ([string]$script:GuiSettings.WebUsername) 190 322 220
    $btnWebPass = New-Object System.Windows.Forms.Button
    $btnWebPass.Text="CHANGE WEB PASSWORD"; $btnWebPass.Location=New-Object System.Drawing.Point(420,318); $btnWebPass.Size=New-Object System.Drawing.Size(160,32)
    $btnWebPass.BackColor=[System.Drawing.Color]::FromArgb(52,152,219); $btnWebPass.ForeColor=[System.Drawing.Color]::White; $btnWebPass.FlatStyle='Flat'
    $f.Controls.Add($btnWebPass)

    $btnSave = New-Object System.Windows.Forms.Button
    $btnSave.Text="SAVE"; $btnSave.Location=New-Object System.Drawing.Point(374,436); $btnSave.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnCancel.Text="CANCEL"; $btnCancel.Location=New-Object System.Drawing.Point(486,436); $btnCancel.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnWebPass.Add_Click({
        $pf = New-Object System.Windows.Forms.Form
        $pf.Text = "Change Web Dashboard Password"
        $pf.Size = New-Object System.Drawing.Size(430, 285)
        $pf.StartPosition = "CenterParent"
        $pf.FormBorderStyle = "FixedDialog"
        $pf.MaximizeBox = $false
        $pf.MinimizeBox = $false
        $pf.BackColor = [System.Drawing.Color]::FromArgb(27,31,36)
        $pf.ForeColor = [System.Drawing.Color]::White
        function Add-PLabel($text,$y){ $l=New-Object System.Windows.Forms.Label; $l.Text=$text; $l.Location=New-Object System.Drawing.Point(24,$y); $l.Size=New-Object System.Drawing.Size(130,22); $pf.Controls.Add($l); return $l }
        function Add-PText($y){ $t=New-Object System.Windows.Forms.TextBox; $t.Location=New-Object System.Drawing.Point(165,$y); $t.Size=New-Object System.Drawing.Size(210,24); $t.UseSystemPasswordChar=$true; $t.BackColor=[System.Drawing.Color]::FromArgb(38,43,51); $t.ForeColor=[System.Drawing.Color]::White; $pf.Controls.Add($t); return $t }
        Add-PLabel "Old Password" 34 | Out-Null; $oldPw = Add-PText 32
        Add-PLabel "New Password" 78 | Out-Null; $newPw = Add-PText 76
        Add-PLabel "Confirm Password" 122 | Out-Null; $confirmPw = Add-PText 120
        $note = New-Object System.Windows.Forms.Label
        $note.Text = "Saved credential applies even when service is stopped. Existing browser sessions will get 401 and must login again."
        $note.Location = New-Object System.Drawing.Point(24,154); $note.Size = New-Object System.Drawing.Size(355,40); $note.ForeColor=[System.Drawing.Color]::Silver
        $pf.Controls.Add($note)
        $ok = New-Object System.Windows.Forms.Button
        $ok.Text="UPDATE"; $ok.Location=New-Object System.Drawing.Point(165,205); $ok.Size=New-Object System.Drawing.Size(95,32); $ok.BackColor=[System.Drawing.Color]::FromArgb(46,204,113); $ok.ForeColor=[System.Drawing.Color]::White; $ok.FlatStyle='Flat'; $pf.Controls.Add($ok)
        $cancel = New-Object System.Windows.Forms.Button
        $cancel.Text="CANCEL"; $cancel.Location=New-Object System.Drawing.Point(280,205); $cancel.Size=New-Object System.Drawing.Size(95,32); $cancel.BackColor=[System.Drawing.Color]::FromArgb(95,101,112); $cancel.ForeColor=[System.Drawing.Color]::White; $cancel.FlatStyle='Flat'; $pf.Controls.Add($cancel)
        $cancel.Add_Click({ $pf.Close() })
        $ok.Add_Click({
            $expectedHash = [string]$script:GuiSettings.WebPasswordHash
            if ([string]::IsNullOrWhiteSpace($expectedHash)) { $expectedHash = Get-Sha256Text "PC_Audit" }
            if ((Get-Sha256Text $oldPw.Text) -ne $expectedHash) { [System.Windows.Forms.MessageBox]::Show("Old password is wrong.", "Password Change", "OK", "Warning") | Out-Null; $oldPw.Clear(); $oldPw.Focus(); return }
            if ([string]::IsNullOrWhiteSpace($newPw.Text) -or $newPw.Text.Length -lt 4) { [System.Windows.Forms.MessageBox]::Show("New password must contain at least 4 characters.", "Password Change", "OK", "Warning") | Out-Null; return }
            if ($newPw.Text -ne $confirmPw.Text) { [System.Windows.Forms.MessageBox]::Show("New password and confirm password do not match.", "Password Change", "OK", "Warning") | Out-Null; return }
            $script:GuiSettings.WebPasswordHash = Get-Sha256Text $newPw.Text
            try { $script:GuiSettings.WebCredentialVersion = [int]$script:GuiSettings.WebCredentialVersion + 1 } catch { $script:GuiSettings.WebCredentialVersion = 2 }
            Save-GuiSettings
            Add-GuiLog "Web dashboard password changed. Existing browser users will receive 401 Unauthorized." "WARN"
            [System.Windows.Forms.MessageBox]::Show("Web dashboard password updated. Existing browser users must login again.", "Password Change", "OK", "Information") | Out-Null
            $pf.Close()
        })
        $pf.ShowDialog($f) | Out-Null
    })
    ...
    $btnSave.Add_Click({
        ...
        $script:GuiSettings.WebUsername = $txtWebUser.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($script:GuiSettings.WebUsername)) { $script:GuiSettings.WebUsername = "PCAudit" }
...
function Reset-GuiInactivityTimer { $script:LastGuiActivity = Get-Date }

function Lock-MainGuiForInactivity {
    if ($script:GuiLocked -or -not $script:MainForm -or $script:MainForm.IsDisposed) { return }
    $script:GuiLocked = $true
    Add-GuiLog "Controller auto-locked after 5 minutes of inactivity. Server state unchanged." "WARN"
    try { $script:MainForm.Hide() } catch { }
    [System.Windows.Forms.MessageBox]::Show("Controller locked due to 5 minutes inactivity. Enter controller login to unlock.", "Auto Lock", "OK", "Information") | Out-Null
    if (Show-LoginWindow) {
        Reset-GuiInactivityTimer
        try { $script:MainForm.Show(); $script:MainForm.WindowState='Normal'; $script:MainForm.Activate() } catch { }
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    $form.Add_MouseMove({ Reset-GuiInactivityTimer })
    $form.Add_MouseDown({ Reset-GuiInactivityTimer })
    $form.Add_KeyDown({ Reset-GuiInactivityTimer })
    $form.Add_Click({ Reset-GuiInactivityTimer })
    foreach ($ctrl in @($form.Controls)) {
        try { $ctrl.Add_MouseMove({ Reset-GuiInactivityTimer }) } catch { }
        try { $ctrl.Add_MouseDown({ Reset-GuiInactivityTimer }) } catch { }
        try { $ctrl.Add_KeyDown({ Reset-GuiInactivityTimer }) } catch { }
        try { $ctrl.Add_Click({ Reset-GuiInactivityTimer }) } catch { }
    }
    ...
    $timer.Add_Tick({
        ...
        if (-not $script:GuiLocked -and $script:MainForm -and $script:MainForm.Visible -and ((Get-Date) - $script:LastGuiActivity).TotalMinutes -ge 5) { Lock-MainGuiForInactivity }
        Update-ButtonState
