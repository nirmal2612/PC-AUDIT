...
.filter-count { color:#d0d6df; font-size:12px; margin-left:10px; }
th.filterable { position:sticky; top:0; z-index:3; vertical-align:middle; }
.th-content { display:inline-flex; align-items:center; justify-content:flex-start; gap:6px; white-space:nowrap; }
.th-label { display:inline-block; }
.filter-btn { display:inline-flex; align-items:center; justify-content:center; border:1px solid rgba(255,255,255,.35); background:#4b5563; color:#fff; border-radius:4px; cursor:pointer; font-size:11px; line-height:1; width:22px; height:20px; padding:0; vertical-align:middle; }
.filter-btn:hover { background:#607086; }
...
function Get-DetailsPage {
   ...
   if ($isPeriodType) {
       ...
       foreach ($r in $records) {
           ...
           $rowHtml = "<tr$rowClass>"
           if ($dupStatus) {
               $rowHtml = $rowHtml + "<td class='dup-cell'>$dupStatus</td>"
           } else {
           ...
           foreach ($col in $stats.OriginalColumns) {
               ...
               $rowHtml = $rowHtml + "<td>$val</td>"
           }
           $rowHtml = $rowHtml + "</tr>"
           $rows = $rows + $rowHtml + [Environment]::NewLine
       }
...
function getCellDisplayValue(cell, headerText) {
    ...
    if (/date|time/i.test(headerText)) {
        var m = txt.match(/\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{2,4}/);
...
function buildTableCache() {
    ...
    tableHeaders = Array.from(table.tHead.rows[0].cells).map(function(th) {
        return th.getAttribute('data-title') || th.innerText.replace('▼','').trim();
    });
    tableCache = Array.from(table.tBodies[0].rows).map(function(row) {
        var vals = tableHeaders.map(function(h, idx) {
            return getCellDisplayValue(row.cells[idx], h);
        });
...
function setupExcelFilters() {
    ...
    headers.forEach(function(th, index) {
        ...
        th.innerHTML = '<span class="th-content"><span class="th-label">' + htmlEscape(title) + '</span></span>';
        ...
        th.querySelector('.th-content').appendChild(btn);
    });
    buildTableCache();
    applyAllFilters();
}
...
function openFilterPopup(colIndex, title, btn) {
    closeFilterPopup();
    if (!tableCache.length) { buildTableCache(); }
    var values = new Map();
    tableCache.forEach(function(item) {
        var val = item.values[colIndex] || '(Blank)';
        values.set(val, (values.get(val) || 0) + 1);
    });

    var selected = activeFilters[colIndex] ? new Set(activeFilters[colIndex]) : new Set(values.keys());
    var popup = document.createElement('div');
    popup.className = 'filter-popup';
    var items = Array.from(values.keys()).sort(function(a,b) { return a.localeCompare(b, undefined, {numeric:true}); });
    ...
    function updateSelectAll() {
        var selectAll = document.getElementById('selectAllFilter');
        if (selectAll) { selectAll.checked = selected.size === values.size; }
    }

    function renderList() {
        var searchBox = document.getElementById('filterItemSearch');
        var q = searchBox ? searchBox.value.toLowerCase() : '';
        var list = document.getElementById('filterList');
        var frag = document.createDocumentFragment();
        list.innerHTML = '';
        items.filter(function(v) { return v.toLowerCase().indexOf(q) >= 0; }).forEach(function(v) {
    ...
    document.getElementById('selectAllFilter').onchange = function() {
        if (this.checked) { items.forEach(function(v) { selected.add(v); }); }
        else { items.forEach(function(v) { selected.delete(v); }); }
        renderList();
    };
    document.getElementById('clearThisFilter').onclick = function() {
    ...
    document.getElementById('applyFilter').onclick = function() {
        if (selected.size === values.size) { delete activeFilters[colIndex]; }
        else { activeFilters[colIndex] = Array.from(selected); }
        closeFilterPopup();
        applyAllFilters();
    };
    renderList();
    setTimeout(function() { var x = document.getElementById('filterItemSearch'); if (x) { x.focus(); } }, 0);
}
...
document.addEventListener('click', function(ev) {
    if (currentPopup && !currentPopup.contains(ev.target)) { closeFilterPopup(); }
});

document.addEventListener('keydown', function(ev) {
    if (ev.ctrlKey && (ev.key === 'f' || ev.key === 'F')) {
        ev.preventDefault();
        var firstBtn = document.querySelector('#t thead .filter-btn');
        if (firstBtn) {
            firstBtn.scrollIntoView({block:'nearest', inline:'nearest'});
            firstBtn.click();
        }
    }
});
...
function applyAllFilters() {
    ...
    var allowedCache = {};
    filterKeys.forEach(function(k) { allowedCache[k] = new Set(activeFilters[k]); });
    ...
    requestAnimationFrame(function() {
        tableCache.forEach(function(item) {
            ...
            for (var i = 0; i < filterKeys.length; i++) {
                ...
                if (!allowedCache[k].has(item.values[idx] || '(Blank)')) {
                    show = false;
                    break;
                }
            }
        ...
        Array.from(document.querySelectorAll('#t thead .filter-btn')).forEach(function(b, idx) {
...
function downloadCSV() {
    ...
    rows.forEach(r => {
        ...
        let colIndex = 0;
        cols.forEach(c => {
            let txt = ((c.tagName === 'TH' && c.getAttribute('data-title')) ? c.getAttribute('data-title') : c.innerText).replace(/▼/g, '').trim().replace(/"/g, '""');
