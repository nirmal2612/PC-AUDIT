...
.th-wrap { display:inline-flex; align-items:center; justify-content:flex-start; gap:4px; white-space:nowrap; }
.th-title { display:inline-block; }
.filter-btn { border:0; border-radius:4px; background:transparent; color:#fff; cursor:pointer; padding:0 2px; margin-left:2px; font-size:12px; line-height:16px; vertical-align:middle; }
...
function htmlSafe(v) { return (v || '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }
function getHeaderText(th) { return (th.getAttribute('data-title') || th.innerText || '').replace('🔽','').trim(); }
function rebuildTableCache() {
    const table = document.getElementById('t');
    cachedHeaders = Array.from(table.tHead.rows[0].cells).map(getHeaderText);
    cachedRows = Array.from(table.tBodies[0].rows).filter(r => r.cells.length > 1).map(r => {
        const values = Array.from(r.cells).map((c, i) => normalizeFilterValue(c.innerText, cachedHeaders[i]));
        return { row: r, values: values };
    });
}
function setupExcelFilters() {
    ...
    Array.from(table.tHead.rows[0].cells).forEach((th, idx) => {
        ...
        th.innerHTML = '<div class="th-wrap"><span class="th-title">' + htmlSafe(title) + '</span><button class="filter-btn" title="Filter">🔽</button></div>';
...
function getUniqueValues(colIndex) {
    const set = new Set();
    cachedRows.forEach(item => set.add(item.values[colIndex] || ''));
...
function renderFilterValues() {
    ...
    list.innerHTML = values.map(v => { const safe = htmlSafe(v); const checked = !selected || selected.has(v) ? 'checked' : ''; return '<label><input type="checkbox" value="' + safe + '" ' + checked + '> ' + (safe || '(blank)') + '</label>'; }).join('') || '<div style="color:#aaa;padding:8px;">No values</div>';
}
...
function applyFilters() {
    let visible = 0, total = cachedRows.length;
    cachedRows.forEach(item => {
        let show = true;
        Object.keys(activeFilters).forEach(k => {
            const idx = parseInt(k,10);
            const val = item.values[idx] || '';
            if (!activeFilters[k].has(val)) show = false;
        });
...
function downloadCSV() {
    ...
    let rows = cachedRows.filter(item => item.row.style.display !== 'none').map(item => item.row);
