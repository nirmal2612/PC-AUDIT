...
$data += $incompleteItems
   ...
   # 📅 Monthly wise PC audit count for dashboard card.
   # Uses all audit rows so every new month is automatically added with year.
   $monthlyAuditCounts = @()
   $monthCulture = [System.Globalization.CultureInfo]::InvariantCulture
   $monthGroups = @($allData | Where-Object { $_.Date } | Group-Object -Property { $_.Date.ToString('yyyy-MM') })
   if ($monthGroups.Count -gt 0) {
       $monthlyAuditCounts = @($monthGroups |
           Sort-Object Name -Descending |
           ForEach-Object {
               $firstDay = [datetime]::ParseExact(($_.Name + '-01'), 'yyyy-MM-dd', $monthCulture)
               [pscustomobject]@{
                   MonthKey   = $_.Name
                   MonthLabel = $firstDay.ToString('MMMM yyyy', $monthCulture)
                   Count      = $_.Count
               }
           })
   }
   ...
   $output = @{
       ...
       MonthlyAuditCounts = $monthlyAuditCounts
       Today = $today
       WeekStart = $weekStart
...
# ====================================================================================================
# 🔐 WEB / CONTROLLER CREDENTIAL HELPERS
# ====================================================================================================
...
function Get-WebUserName {
   if ($script:GuiSettings -and -not [string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebUserName)) { return [string]$script:GuiSettings.WebUserName }
   return "PCAudit"
}

function Get-WebPassword {
   if ($script:GuiSettings -and -not [string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebPassword64)) {
      $p = ConvertFrom-Base64Text ([string]$script:GuiSettings.WebPassword64)
...
function Reset-WebAuthSessionGeneration {
   # New generation invalidates browser-auth cache/challenge after credential change or service restart.
   try { $script:WebAuthSessionGeneration = ([guid]::NewGuid().ToString('N')) } catch { $script:WebAuthSessionGeneration = [string](Get-Date -Format 'yyyyMMddHHmmssfff') }
}

function Get-WebAuthRealm {
   $version = "1"
   try {
      if ($script:GuiSettings -and -not [string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebAuthVersion)) { $version = [string]$script:GuiSettings.WebAuthVersion }
   } catch { }
   if ([string]::IsNullOrWhiteSpace([string]$script:WebAuthSessionGeneration)) { Reset-WebAuthSessionGeneration }
   return "PC Audit Dashboard-$version-$($script:WebAuthSessionGeneration)"
}

function Get-ControllerUserName {
   if ($script:GuiSettings -and -not [string]::IsNullOrWhiteSpace([string]$script:GuiSettings.ControllerUserName)) { return [string]$script:GuiSettings.ControllerUserName }
   return "HH0010520"
}

function Get-ControllerPassword {
   if ($script:GuiSettings -and -not [string]::IsNullOrWhiteSpace([string]$script:GuiSettings.ControllerPassword64)) {
      $p = ConvertFrom-Base64Text ([string]$script:GuiSettings.ControllerPassword64)
      if (-not [string]::IsNullOrEmpty($p)) { return $p }
   }
   return "Foxconn-FXCN-IT"
}

function Test-WebBasicAuth {
   param($Context)
   try {
      $auth = [string]$Context.Request.Headers['Authorization']
      if ([string]::IsNullOrWhiteSpace($auth) -or -not $auth.StartsWith('Basic ')) { return $false }
      $raw = [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($auth.Substring(6)))
      $idx = $raw.IndexOf(':')
      if ($idx -lt 0) { return $false }
      $u = $raw.Substring(0,$idx)
      $p = $raw.Substring($idx+1)
      return ($u -eq (Get-WebUserName) -and $p -eq (Get-WebPassword))
   } catch { return $false }
}

function Send-WebAuthRequired {
   param($Context)
   try {
      $Context.Response.StatusCode = 401
      $Context.Response.Headers.Add('WWW-Authenticate',('Basic realm="' + (Get-WebAuthRealm) + '"'))
      $msg = "Authentication required for PC Audit Dashboard."
...
function Get-ExcelFilterHeaderHtml {
   param([string[]]$Columns)
   $i = 0
   return (($Columns | ForEach-Object {
      $name = Get-HtmlEncoded $_
      $html = "<th class='filterable' data-col='$i'><span class='th-text'>$name</span><button class='filter-btn' onclick='openColumnFilter(event,$i)' title='Filter'>▼</button></th>"
      $i++
      $html
   }) -join "")
}

function Get-NormalizedFilterValue {
   param([string]$ColumnName, [string]$Value)
   if ([string]::IsNullOrWhiteSpace($Value)) { return "(Blanks)" }
   if ($ColumnName -match '(?i)date|time') {
      try {
         $dt = [datetime]::Parse($Value, [System.Globalization.CultureInfo]::InvariantCulture)
         return $dt.ToString('yyyy-MM-dd')
      } catch {
         try { return ([datetime]$Value).ToString('yyyy-MM-dd') } catch { return $Value.Trim() }
      }
   }
   return $Value.Trim()
}

function Get-ExcelFilterCssJs {
return @"
<style>
th.filterable { position: sticky; top: 0; padding-right: 34px; user-select:none; }
.th-text { display:inline-block; max-width: 240px; overflow:hidden; text-overflow:ellipsis; vertical-align:middle; }
.filter-btn { position:absolute; right:6px; top:50%; transform:translateY(-50%); width:22px; height:22px; border:1px solid #5b6575; border-radius:4px; background:#242a33; color:#dce6f1; cursor:pointer; font-size:11px; line-height:18px; }
.filter-btn:hover, .filter-btn.active { background:#3498db; color:#fff; border-color:#3498db; }
.excel-filter-pop { position:fixed; z-index:99999; width:260px; max-height:390px; background:#202631; border:1px solid #566173; border-radius:8px; box-shadow:0 14px 35px rgba(0,0,0,.55); padding:10px; display:none; color:#fff; font-family:'Segoe UI',Arial,sans-serif; }
.excel-filter-pop input[type='text'] { width:100%; padding:8px 10px; border-radius:6px; border:1px solid #596579; background:#10141b; color:#fff; outline:none; margin-bottom:8px; }
.filter-list { max-height:235px; overflow:auto; border:1px solid #3d4655; border-radius:6px; background:#151a22; padding:4px; }
.filter-item { display:block; padding:5px 6px; font-size:12px; color:#eef3f9; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.filter-item:hover { background:#2f3846; }
.filter-actions { display:flex; gap:7px; margin-top:9px; }
.filter-actions button { flex:1; padding:7px 8px; border:0; border-radius:6px; cursor:pointer; font-weight:700; }
.filter-ok { background:#27ae60; color:#fff; }
.filter-clear { background:#e67e22; color:#fff; }
.filter-all { margin:4px 0 8px 0; font-size:12px; display:block; }
.filter-summary { color:#aab6c8; font-size:12px; margin-left:10px; }
</style>
<script>
let excelFilterState = {};
let excelFilterColumn = -1;
let excelFilterPopup = null;
let excelFilterTable = null;
let excelFilterSearchTimer = null;

function getCellFilterValue(cell){
    if(!cell) return '(Blanks)';
    let v = cell.getAttribute('data-filter-value');
    if(v === null || v === undefined) v = cell.innerText;
    v = (v || '').trim();
    return v === '' ? '(Blanks)' : v;
}
function uniqueColumnValues(col){
    const values = new Set();
    document.querySelectorAll('#t tbody tr').forEach(r => values.add(getCellFilterValue(r.cells[col])));
...
function ensureExcelFilterPopup(){
    if(excelFilterPopup) return excelFilterPopup;
    const p = document.createElement('div');
    p.className = 'excel-filter-pop';
    p.innerHTML = "<input id='filterSearchBox' type='text' placeholder='Search'>"+
                  "<label class='filter-all'><input id='filterSelectAll' type='checkbox'> Select All</label>"+
                  "<div id='filterList' class='filter-list'></div>"+
                  "<div class='filter-actions'><button class='filter-ok' onclick='applyColumnFilter()'>OK</button><button class='filter-clear' onclick='clearColumnFilter()'>Clear</button></div>";
    document.body.appendChild(p);
    document.getElementById('filterSearchBox').addEventListener('input', function(){ clearTimeout(excelFilterSearchTimer); excelFilterSearchTimer = setTimeout(renderColumnFilterList, 80); });
    document.getElementById('filterSelectAll').addEventListener('change', function(){ document.querySelectorAll('#filterList input[type=checkbox]').forEach(cb => cb.checked = this.checked); });
    excelFilterPopup = p;
    return p;
}
function openColumnFilter(ev, col){
    ev.stopPropagation();
    excelFilterTable = document.getElementById('t');
    excelFilterColumn = col;
    const p = ensureExcelFilterPopup();
    const rect = ev.target.getBoundingClientRect();
    p.style.left = Math.min(rect.left, window.innerWidth - 280) + 'px';
    p.style.top = Math.min(rect.bottom + 4, window.innerHeight - 410) + 'px';
    p.style.display = 'block';
    document.getElementById('filterSearchBox').value = '';
    renderColumnFilterList();
}
function renderColumnFilterList(){
    const q = (document.getElementById('filterSearchBox').value || '').toLowerCase();
    const list = document.getElementById('filterList');
    const selected = excelFilterState[excelFilterColumn] || null;
    const vals = uniqueColumnValues(excelFilterColumn).filter(v => v.toLowerCase().indexOf(q) !== -1);
    let html = '';
    vals.forEach(v => {
        const safe = v.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
        const checked = (!selected || selected.has(v)) ? 'checked' : '';
        html += `<label class='filter-item' title="${safe}"><input type='checkbox' value="${safe}" ${checked}> ${safe}</label>`;
    });
    list.innerHTML = html || "<div style='color:#999;padding:8px;'>No values</div>";
    const boxes = Array.from(list.querySelectorAll('input[type=checkbox]'));
    document.getElementById('filterSelectAll').checked = boxes.length > 0 && boxes.every(cb => cb.checked);
}
function applyColumnFilter(){
    const checked = Array.from(document.querySelectorAll('#filterList input[type=checkbox]:checked')).map(cb => cb.value);
    const all = uniqueColumnValues(excelFilterColumn);
    if(checked.length === 0) excelFilterState[excelFilterColumn] = new Set(['__NO_MATCH__']);
    else if(checked.length === all.length) delete excelFilterState[excelFilterColumn];
    else excelFilterState[excelFilterColumn] = new Set(checked);
    filterRowsFast();
    closeExcelFilterPopup();
}
function clearColumnFilter(){
    delete excelFilterState[excelFilterColumn];
    filterRowsFast();
    closeExcelFilterPopup();
}
function filterRowsFast(){
    const rows = document.querySelectorAll('#t tbody tr');
    let visible = 0;
    rows.forEach(r => {
        let show = true;
        for(const c in excelFilterState){
            const allowed = excelFilterState[c];
            if(allowed && !allowed.has(getCellFilterValue(r.cells[parseInt(c)]))){ show = false; break; }
        }
        r.style.display = show ? '' : 'none';
        if(show) visible++;
    });
    document.querySelectorAll('th.filterable .filter-btn').forEach((b,idx)=> b.classList.toggle('active', !!excelFilterState[idx]));
    const info = document.getElementById('filterCountInfo');
    if(info) info.innerText = 'Visible: ' + visible;
}
function closeExcelFilterPopup(){ if(excelFilterPopup) excelFilterPopup.style.display='none'; }
document.addEventListener('click', function(e){ if(excelFilterPopup && !excelFilterPopup.contains(e.target) && !e.target.classList.contains('filter-btn')) closeExcelFilterPopup(); });
</script>
"@
}
...
function Get-MainPage {
   ...
   # 📅 Build Monthly PC Audit Count rows.
   # Clicking a month opens details for only that month.
   $monthRows = ""
   if ($stats.MonthlyAuditCounts -and $stats.MonthlyAuditCounts.Count -gt 0) {
       foreach ($m in $stats.MonthlyAuditCounts) {
           $monthLabel = Get-HtmlEncoded $m.MonthLabel
           $monthKey = Get-HtmlEncoded $m.MonthKey
           $monthRows += "<tr class='clickable-row' onclick=\"openDetail('Month_$monthKey')\"><td>$monthLabel</td><td>$($m.Count)</td></tr>`n"
       }
   } else {
       $monthRows = "<tr><td colspan='2' style='color:#888;'>No monthly data available</td></tr>"
   }
...
.month-audit-scroll::-webkit-scrollbar { width: 7px; }
.month-audit-scroll::-webkit-scrollbar-track { background: #242830; border-radius: 10px; }
.month-audit-scroll::-webkit-scrollbar-thumb { background: #566070; border-radius: 10px; }

.clickable-row { cursor: pointer; }
.clickable-row:hover { background: #3f4652 !important; }
...
@media (max-width: 900px) { 
    ...
    .left-card-stack { min-height: auto; }
...
<div class="content-grid">
    ...
    <div class="left-card-stack">
        ...
        <div class="section">
            <h2>📅 Monthly PC Audit Count</h2>
            <div class="month-audit-scroll">
                <table>
                    <thead>
                        <tr>
                            <th>Month</th>
...
function Get-DetailsPage {
   ...
   $monthDetailKey = $null
   if ($Type -match '^Month_(\d{4}-\d{2})$') {
       $baseType = 'MonthDetail'
       $monthDetailKey = $Matches[1]
   }
   ...
   $isPeriodType = @('Daily', 'Weekly', 'Monthly', 'Total', 'MonthDetail') -contains $baseType
   ...
   if ($isPeriodType) {
       ...
       switch ($baseType) {
           ...
           'MonthDetail' {
               $monthStartDate = [datetime]::ParseExact(($monthDetailKey + '-01'), 'yyyy-MM-dd', [System.Globalization.CultureInfo]::InvariantCulture)
               $monthEndDate = $monthStartDate.AddMonths(1).AddDays(-1)
               $monthLabel = $monthStartDate.ToString('MMMM yyyy', [System.Globalization.CultureInfo]::InvariantCulture)
               $records = @($stats.Data | Where-Object { 
                   $_.Date -and $_.Date.ToString('yyyy-MM') -eq $monthDetailKey
               })
               $title = "PC Audit Records - $monthLabel"
               $dateRangeInfo = "Month: $($monthStartDate.ToString('yyyy-MM-dd')) to $($monthEndDate.ToString('yyyy-MM-dd')) | Filter: All Networks"
           }
           'Total' {
               $records = @($stats.Data | Where-Object { 
                   -not $segmentFilter -or $_.NetworkSegment -eq $segmentFilter
       ...
       $hdr = Get-ExcelFilterHeaderHtml -Columns $columns
       ...
       foreach ($r in $records) {
           ...
           $rowHtml = "<tr$rowClass>"
           $dupFilterVal = if ($dupStatus) { $dupStatus } else { "(Blanks)" }
           $rowHtml += if ($dupStatus) { "<td class='dup-cell' data-filter-value='$dupFilterVal'>$dupStatus</td>" } else { "<td data-filter-value='(Blanks)'></td>" }
           ...
           foreach ($col in $stats.OriginalColumns) {
               ...
               if ($col -eq "SNo" -or $col -eq "S.No" -or $col -eq "SNO" -or $col -eq "SerialNo" -or $col.ToLower().Replace(" ", "").Replace(".", "") -eq "sno") {
                   $rawVal = $seqNum.ToString()
               }
               elseif ($r._OriginalRow -and $r._OriginalRow.$col) {
                   $rawVal = $r._OriginalRow.$col.ToString()
               }
               $val = Get-HtmlEncoded $rawVal
               $filterVal = Get-HtmlEncoded (Get-NormalizedFilterValue -ColumnName $col -Value $rawVal)
               $rowHtml += "<td data-filter-value='$filterVal'>$val</td>"
           }
           $rowHtml += "</tr>"
           $rows += "$rowHtml`n"
       }
       ...
       $filterAssets = Get-ExcelFilterCssJs
       $filterAssets = Get-ExcelFilterCssJs
       $html = @"
<!DOCTYPE html>
<html>
...
<div class="info">$(($records).Count) records found | $colCount columns <span id="filterCountInfo" class="filter-summary"></span></div>
...
function downloadCSV() {
    ...
    rows.forEach(r => {
        if (r.parentElement && r.parentElement.tagName === 'TBODY' && r.style.display === 'none') { return; }
        let row = [];
        let cols = r.querySelectorAll('td, th');
        let isDuplicate = r.classList.contains('duplicate');
        let colIndex = 0;
        cols.forEach(c => {
            let txt = c.innerText.replace(/▼/g, '').trim().replace(/"/g, '""');
...
"@
   ...
   elseif ($isIssueType) {
       ...
       $issueColumns = @("SNo", "Computer Name", "IP Address", "Network", $statusColumnName, "Auditor ID", "Audit Date")
       $hdr = Get-ExcelFilterHeaderHtml -Columns $issueColumns
       ...
       foreach ($r in $records) {
           ...
           $auditDateFilter = Get-HtmlEncoded (Get-NormalizedFilterValue -ColumnName "Audit Date" -Value $auditDate)
           $rows += "<tr><td data-filter-value='$seqNum'>$seqNum</td><td data-filter-value='$computerName'>$computerName</td><td data-filter-value='$ipAddress'>$ipAddress</td><td data-filter-value='$network'>$network</td><td class='status-value' data-filter-value='$statusValueHtml'>$statusValueHtml</td><td data-filter-value='$auditorId'>$auditorId</td><td data-filter-value='$auditDateFilter'>$auditDate</td></tr>`n"
       }
...
<div class="info">$(($records).Count) records found <span id="filterCountInfo" class="filter-summary"></span></div>
<button class="export" onclick="downloadCSV()">Export CSV</button>
<button class="close" onclick="window.close()">Close</button>
<div class="table-container">
<table id="t">
...
function Start-DashboardServer {
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           if ($path -ne '/PC Audit Dashbaord Service Controller.ico' -and -not (Test-WebBasicAuth -Context $context)) {
               Send-WebAuthRequired -Context $context
               continue
           }
...
function Initialize-GuiSettings {
    $defaults = [ordered]@{
        ...
        AutoRefreshLog = $true
        WebUserName = "PCAudit"
        WebPassword64 = (ConvertTo-Base64Text "PC_Audit")
        WebAuthVersion = ([guid]::NewGuid().ToString('N'))
        ControllerUserName = "HH0010520"
        ControllerPassword64 = (ConvertTo-Base64Text "Foxconn-FXCN-IT")
        AutoLockMinutes = 5
    }
...
function Start-GuiDashboardListener {
    ...
    Reset-WebAuthSessionGeneration
...
function Update-ButtonState {
    ...
    if ($script:BtnStart) { $script:BtnStart.Enabled = -not $running }
    if ($script:BtnStop) { $script:BtnStop.Enabled = $running }
    if ($script:BtnRestart) { $script:BtnRestart.Enabled = $true }
    if ($script:ControllerLocked) {
        if ($script:BtnStart) { $script:BtnStart.Enabled = $false }
...
function Show-PasswordChangeWindow {
    param([ValidateSet('Web','Controller')][string]$Mode)
    if (Is-DashboardRunning) {
        [System.Windows.Forms.MessageBox]::Show("Please STOP the dashboard service before changing password.", "Password Change", "OK", "Information") | Out-Null
        return
    }
    $isWeb = ($Mode -eq 'Web')
    $currentUser = if ($isWeb) { Get-WebUserName } else { Get-ControllerUserName }
    $currentPass = if ($isWeb) { Get-WebPassword } else { Get-ControllerPassword }
    $pf = New-Object System.Windows.Forms.Form
    $pf.Text = "$Mode Password Change"
    $pf.Size = New-Object System.Drawing.Size(420,260)
    $pf.StartPosition = "CenterParent"
    $pf.FormBorderStyle = "FixedDialog"
    $pf.MaximizeBox = $false
    $pf.MinimizeBox = $false
    $pf.BackColor = [System.Drawing.Color]::FromArgb(24,28,36)
    $pf.ForeColor = [System.Drawing.Color]::White
    $pf.Font = New-Object System.Drawing.Font("Segoe UI", 9)
    function L($t,$y){ $l=New-Object System.Windows.Forms.Label; $l.Text=$t; $l.Location=New-Object System.Drawing.Point(24,$y); $l.Size=New-Object System.Drawing.Size(135,22); $pf.Controls.Add($l); return $l }
    function T($v,$y,$pwd=$false){ $t=New-Object System.Windows.Forms.TextBox; $t.Text=$v; $t.Location=New-Object System.Drawing.Point(165,$y); $t.Size=New-Object System.Drawing.Size(205,24); $t.UseSystemPasswordChar=$pwd; $pf.Controls.Add($t); return $t }
    L "Username" 25; $u=T $currentUser 23 $false
    L "Old Password" 62; $old=T "" 60 $true
    L "New Password" 99; $new=T "" 97 $true
    L "Confirm Password" 136; $confirm=T "" 134 $true
    $ok=New-Object System.Windows.Forms.Button; $ok.Text="Save"; $ok.Location=New-Object System.Drawing.Point(205,178); $ok.Size=New-Object System.Drawing.Size(80,30); $ok.BackColor=[System.Drawing.Color]::FromArgb(39,174,96); $ok.ForeColor=[System.Drawing.Color]::White; $ok.FlatStyle='Flat'; $pf.Controls.Add($ok)
    $cancel=New-Object System.Windows.Forms.Button; $cancel.Text="Cancel"; $cancel.Location=New-Object System.Drawing.Point(295,178); $cancel.Size=New-Object System.Drawing.Size(80,30); $cancel.BackColor=[System.Drawing.Color]::FromArgb(95,105,118); $cancel.ForeColor=[System.Drawing.Color]::White; $cancel.FlatStyle='Flat'; $cancel.Add_Click({$pf.Close()}); $pf.Controls.Add($cancel)
    $ok.Add_Click({
        if ($old.Text -ne $currentPass) { [System.Windows.Forms.MessageBox]::Show("Old password is wrong.", "Password Change", "OK", "Warning") | Out-Null; $old.Focus(); return }
        if ([string]::IsNullOrWhiteSpace($u.Text) -or [string]::IsNullOrWhiteSpace($new.Text)) { [System.Windows.Forms.MessageBox]::Show("Username and new password cannot be empty.", "Password Change", "OK", "Warning") | Out-Null; return }
        if ($new.Text -ne $confirm.Text) { [System.Windows.Forms.MessageBox]::Show("New password and confirm password do not match.", "Password Change", "OK", "Warning") | Out-Null; return }
        if ($isWeb) {
            $script:GuiSettings.WebUserName=$u.Text.Trim();
            $script:GuiSettings.WebPassword64=(ConvertTo-Base64Text $new.Text);
            $script:GuiSettings.WebAuthVersion=([guid]::NewGuid().ToString('N'));
            Reset-WebAuthSessionGeneration
        }
        else { $script:GuiSettings.ControllerUserName=$u.Text.Trim(); $script:GuiSettings.ControllerPassword64=(ConvertTo-Base64Text $new.Text) }
        Save-GuiSettings
        [System.Windows.Forms.MessageBox]::Show("$Mode credential changed successfully. Start the service and use the new credential.", "Password Change", "OK", "Information") | Out-Null
        $pf.Close()
    })
...
function Show-SettingsWindow {
    ...
    New-Label "Auto Lock Minutes" 24 322 | Out-Null
    $txtAutoLock = New-Text ([string]$script:GuiSettings.AutoLockMinutes) 190 320 80

    New-Label "Credential Change" 24 356 | Out-Null
    $btnWebPwd = New-Object System.Windows.Forms.Button
    $btnWebPwd.Text = "WEB PASSWORD"
    $btnWebPwd.Location = New-Object System.Drawing.Point(190,352)
    $btnWebPwd.Size = New-Object System.Drawing.Size(145,30)
    $btnWebPwd.BackColor = [System.Drawing.Color]::FromArgb(42,125,185)
    $btnWebPwd.ForeColor = [System.Drawing.Color]::White
    $btnWebPwd.FlatStyle = 'Flat'
    $f.Controls.Add($btnWebPwd)

    $btnControllerPwd = New-Object System.Windows.Forms.Button
    $btnControllerPwd.Text = "CONTROLLER PASSWORD"
    $btnControllerPwd.Location = New-Object System.Drawing.Point(345,352)
    $btnControllerPwd.Size = New-Object System.Drawing.Size(175,30)
    $btnControllerPwd.BackColor = [System.Drawing.Color]::FromArgb(105,75,165)
    $btnControllerPwd.ForeColor = [System.Drawing.Color]::White
    $btnControllerPwd.FlatStyle = 'Flat'
    $f.Controls.Add($btnControllerPwd)

    $btnSave = New-Object System.Windows.Forms.Button
    $btnSave.Text="SAVE"; $btnSave.Location=New-Object System.Drawing.Point(374,405); $btnSave.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnCancel.Text="CANCEL"; $btnCancel.Location=New-Object System.Drawing.Point(486,405); $btnCancel.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnWebPwd.Add_Click({ Show-PasswordChangeWindow -Mode Web })
    $btnControllerPwd.Add_Click({ Show-PasswordChangeWindow -Mode Controller })
    ...
    $btnSave.Add_Click({
        ...
        $lockMin = 5
        [void][int]::TryParse($txtAutoLock.Text.Trim(), [ref]$lockMin)
        if ($lockMin -lt 1) { $lockMin = 5 }
        $script:GuiSettings.AutoLockMinutes = $lockMin
        Save-GuiSettings
        Apply-GuiSettings
...
function Show-LoginWindow {
    ...
    $txtUser.Location=New-Object System.Drawing.Point(150,86); $txtUser.Size=New-Object System.Drawing.Size(200,25); $txtUser.Text=(Get-ControllerUserName)
    ...
    $btnLogin.Add_Click({
        if ($txtUser.Text.Trim() -eq (Get-ControllerUserName) -and $txtPass.Text -eq (Get-ControllerPassword)) {
            $script:LoginOk = $true
            $login.Close()
        } else {
...
function Show-MainGui {
    ...
    $script:LastControllerActivity = Get-Date
    function Touch-ControllerActivity { $script:LastControllerActivity = Get-Date }
    function Lock-ControllerWindow {
        if ($script:ControllerLocked) { return }
        $script:ControllerLocked = $true
        if ($script:BtnStart) { $script:BtnStart.Enabled = $false }
        if ($script:BtnStop) { $script:BtnStop.Enabled = $false }
        if ($script:BtnRestart) { $script:BtnRestart.Enabled = $false }
        Add-GuiLog "Controller auto-locked due to inactivity." "WARN"
        [System.Windows.Forms.MessageBox]::Show("Controller is locked due to inactivity. Login again to unlock.", "Auto Lock", "OK", "Information") | Out-Null
        if (Show-LoginWindow) { $script:ControllerLocked = $false; Touch-ControllerActivity; Update-ButtonState; Add-GuiLog "Controller unlocked." "INFO" }
    }
    $form.Add_MouseMove({ Touch-ControllerActivity })
    $form.Add_MouseDown({ Touch-ControllerActivity })
    $form.Add_KeyDown({ Touch-ControllerActivity })
    ...
    $btnTest           = New-CompactButton "TEST"     244 72 ([System.Drawing.Color]::FromArgb(105,75,165))
    $btnOpen           = New-CompactButton "OPEN"     322 72 ([System.Drawing.Color]::FromArgb(42,125,185))
    $btnSettings       = New-CompactButton "SETTINGS" 400 86 ([System.Drawing.Color]::FromArgb(63,72,84))
    $btnExit           = New-CompactButton "EXIT"     492 72 ([System.Drawing.Color]::FromArgb(92,101,112))
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    $miStart = $trayMenu.Items.Add("Start Server")
    $miStop = $trayMenu.Items.Add("Stop Server")
    $miRestart = $trayMenu.Items.Add("Restart Server")
    $miTest = $trayMenu.Items.Add("Quick Test")
    $miSettings = $trayMenu.Items.Add("Settings")
    ...
    $miStart.Add_Click({ Start-GuiDashboard })
    $miStop.Add_Click({ Stop-GuiDashboard })
    $miRestart.Add_Click({ Restart-GuiDashboard })
    $miTest.Add_Click({ Test-GuiDashboardUrl })
    $miSettings.Add_Click({ $form.Show(); $form.WindowState = 'Normal'; $form.Activate(); Show-SettingsWindow })
    $miExit.Add_Click({ Stop-GuiDashboard; $script:NotifyIcon.Visible=$false; $form.Close() })

    $script:BtnStart.Add_Click({ Touch-ControllerActivity; if (-not $script:ControllerLocked) { Start-GuiDashboard } })
    $script:BtnStop.Add_Click({ Touch-ControllerActivity; if (-not $script:ControllerLocked) { Stop-GuiDashboard } })
    $script:BtnRestart.Add_Click({ Touch-ControllerActivity; if (-not $script:ControllerLocked) { Restart-GuiDashboard } })
    $btnOpen.Add_Click({ Open-GuiDashboard })
    $btnTest.Add_Click({ Test-GuiDashboardUrl })
    $btnSettings.Add_Click({ Touch-ControllerActivity; if (-not $script:ControllerLocked) { Show-SettingsWindow } })
    $btnExit.Add_Click({ Stop-GuiDashboard; $script:NotifyIcon.Visible=$false; $form.Close() })
    ...
    $timer.Add_Tick({
        ...
        $lockMinutes = 5
        try { $lockMinutes = [int]$script:GuiSettings.AutoLockMinutes } catch { $lockMinutes = 5 }
        if (-not $script:ControllerLocked -and ((Get-Date) - $script:LastControllerActivity).TotalMinutes -ge $lockMinutes) { Lock-ControllerWindow }
        Update-ButtonState
