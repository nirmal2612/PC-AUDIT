...
# ====================================================================================================
# WEB AUTHENTICATION + NO-CACHE SETTINGS
# ====================================================================================================
# Purpose:
# • Protect dashboard URL with login credentials.
# • Prevent browser/proxy cache from serving stale dashboard pages.
# • Keep filter/search actions fast by doing filtering in browser without reloading the CSV.
# ----------------------------------------------------------------------------------------------------
$script:WebAuthUsername = "PCAudit"
$script:WebAuthPassword = "PC_Audit"
$script:WebAuthCookieName = "PCAUDIT_DASHBOARD_AUTH"
$script:WebAuthToken = ([Guid]::NewGuid().ToString("N"))
...
.filter-bar{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:0 0 14px 0}.quick-search{min-width:280px;max-width:420px;flex:1;padding:10px 12px;border:1px solid #4b5565;border-radius:7px;background:#15191f;color:#fff;outline:none}.quick-search:focus{border-color:#4da3ff}.filter-note{font-size:12px;color:#9aa6b5}.filter-btn{margin-left:6px;border:1px solid #667085;background:#4a515e;color:#fff;border-radius:4px;padding:2px 6px;cursor:pointer;font-size:11px;line-height:1}.filter-btn.active{background:#2ecc71;border-color:#2ecc71;color:#fff}.excel-filter{position:fixed;z-index:99999;width:280px;max-height:390px;background:#fff;color:#111;border:1px solid #999;border-radius:4px;box-shadow:0 10px 30px rgba(0,0,0,.45);padding:9px;font-family:'Segoe UI',Arial,sans-serif;font-size:12px}.excel-filter input[type=text]{width:100%;padding:7px;border:1px solid #aaa;border-radius:3px;margin-bottom:7px}.excel-filter .actions{display:flex;gap:6px;margin-top:8px}.excel-filter button{padding:6px 10px;border:1px solid #777;border-radius:3px;cursor:pointer;background:#f2f2f2;color:#111}.excel-filter button.primary{background:#217346;color:#fff;border-color:#217346}.excel-filter .list{max-height:220px;overflow:auto;border:1px solid #ddd;padding:5px;background:#fff}.excel-filter label{display:block;color:#111;font-weight:400;padding:3px 2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.excel-filter .select-all{border-bottom:1px solid #ddd;margin-bottom:4px;padding-bottom:5px}.hidden-by-filter{display:none!important}th{user-select:none}.row-count-badge{color:#2ecc71;font-weight:600}
</style>
</head>
<body>
...
<div class="filter-bar"><input id="quickSearch" class="quick-search" placeholder="Keyword search..." oninput="applyAllFilters()"><span class="filter-note">Excel-like column filters: click 🔽 in header. Date columns show date only.</span><span id="rowCountBadge" class="row-count-badge"></span></div>
...
<script>
var columnFilters = {};
function normalizeDateIfNeeded(text, header){
    text = (text || '').trim(); header = (header || '').toLowerCase();
    if(header.indexOf('date') >= 0 || header.indexOf('time') >= 0){
        var m = text.match(/\d{4}-\d{2}-\d{2}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4}/);
        if(m){ return m[0]; }
    }
    return text;
}
function setupExcelFilters(){
    var table = document.getElementById('t'); if(!table || !table.tHead){return;}
    var ths = table.tHead.rows[0].cells;
    for(var i=0;i<ths.length;i++){
        (function(idx){
            var th = ths[idx];
            var headerText = th.childNodes.length ? th.childNodes[0].textContent.trim() : th.textContent.trim();
            th.setAttribute('data-title', headerText);
            var btn = document.createElement('button');
            btn.className = 'filter-btn';
            btn.type = 'button';
            btn.title = 'Filter ' + headerText;
            btn.textContent = '🔽';
            btn.onclick = function(ev){ ev.stopPropagation(); showFilterDropdown(idx, btn); };
            th.appendChild(btn);
        })(i);
    }
    applyAllFilters();
...
function getColumnValues(col){
    var table = document.getElementById('t'); var vals = {}; var out=[];
    var header = table.tHead.rows[0].cells[col].getAttribute('data-title') || '';
    for(var i=0;i<table.tBodies[0].rows.length;i++){
        var row = table.tBodies[0].rows[i];
        var val = row.cells[col] ? normalizeDateIfNeeded(row.cells[col].innerText, header) : '';
        if(!vals[val]){ vals[val]=true; out.push(val); }
    }
...
function showFilterDropdown(col, anchor){
    ...
    var selected = columnFilters[col] ? columnFilters[col] : null;
    var box = document.createElement('div'); box.className='excel-filter'; box.id='excelFilterBox';
    var html = '<input type="text" id="filterSearchBox" placeholder="Search..." oninput="filterDropdownList()">';
    html += '<label class="select-all"><input type="checkbox" id="filterSelectAll" checked onchange="toggleFilterSelectAll(this.checked)"> Select All</label>';
    html += '<div class="list" id="filterValueList">';
    for(var i=0;i<values.length;i++){
        var safe = String(values[i]).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
        var checked = (!selected || selected[String(values[i])]) ? 'checked' : '';
        html += '<label data-val="'+safe.toLowerCase()+'"><input type="checkbox" class="filterValue" value="'+safe+'" '+checked+'> '+(safe || '(Blank)')+'</label>';
    }
    html += '</div><div class="actions"><button class="primary" onclick="applyColumnFilter('+col+')">OK</button><button onclick="clearColumnFilter('+col+')">Clear</button><button onclick="closeFilterDropdown()">Cancel</button></div>';
    box.innerHTML = html; document.body.appendChild(box);
    var r = anchor.getBoundingClientRect(); box.style.left = Math.min(r.left, window.innerWidth-300) + 'px'; box.style.top = (r.bottom+4) + 'px';
    setTimeout(function(){var s=document.getElementById('filterSearchBox'); if(s){s.focus();}},30);
}
function filterDropdownList(){
    var q = (document.getElementById('filterSearchBox').value || '').toLowerCase();
    document.querySelectorAll('#filterValueList label').forEach(function(l){ l.style.display = l.getAttribute('data-val').indexOf(q)>=0 ? 'block':'none'; });
}
function toggleFilterSelectAll(state){ document.querySelectorAll('#filterValueList .filterValue').forEach(function(c){ if(c.closest('label').style.display !== 'none'){ c.checked = state; } }); }
function applyColumnFilter(col){
    var selected = {}; var count=0; var total=0;
    document.querySelectorAll('#filterValueList .filterValue').forEach(function(c){ total++; if(c.checked){ selected[c.value]=true; count++; } });
    if(count === total){ delete columnFilters[col]; } else { columnFilters[col] = selected; }
    closeFilterDropdown(); updateFilterButtons(); applyAllFilters();
}
function clearColumnFilter(col){ delete columnFilters[col]; closeFilterDropdown(); updateFilterButtons(); applyAllFilters(); }
function closeFilterDropdown(){ var old=document.getElementById('excelFilterBox'); if(old){old.remove();} }
function updateFilterButtons(){
    var table=document.getElementById('t'); if(!table || !table.tHead){return;}
    var ths=table.tHead.rows[0].cells;
    for(var i=0;i<ths.length;i++){ var b=ths[i].querySelector('.filter-btn'); if(b){ if(columnFilters[i]){b.classList.add('active');}else{b.classList.remove('active');} } }
}
function applyAllFilters(){
    var table=document.getElementById('t'); if(!table || !table.tBodies.length){return;}
    var qEl=document.getElementById('quickSearch'); var q=qEl ? qEl.value.toLowerCase().trim() : '';
    var rows=table.tBodies[0].rows; var visible=0;
    for(var r=0;r<rows.length;r++){
        var row=rows[r]; var show=true;
        if(q && row.innerText.toLowerCase().indexOf(q)<0){ show=false; }
        if(show){
            for(var col in columnFilters){
                var header=table.tHead.rows[0].cells[col].getAttribute('data-title') || '';
                var cell=row.cells[col]; var val=cell ? normalizeDateIfNeeded(cell.innerText, header) : '';
                if(!columnFilters[col][val]){ show=false; break; }
            }
        }
        row.classList.toggle('hidden-by-filter', !show);
        if(show){visible++;}
    }
    var badge=document.getElementById('rowCountBadge'); if(badge){badge.textContent='Showing '+visible+' records';}
}
document.addEventListener('click', function(e){ var box=document.getElementById('excelFilterBox'); if(box && !box.contains(e.target) && !e.target.classList.contains('filter-btn')){ closeFilterDropdown(); } });
window.addEventListener('load', setupExcelFilters);
</script>
...
# ====================================================================================================
# WEB LOGIN / AUTHENTICATION HELPERS
# ====================================================================================================
...
function Test-WebAuthenticated {
    param($Context)
    try {
        $cookie = $Context.Request.Cookies[$script:WebAuthCookieName]
        return ($cookie -and $cookie.Value -eq $script:WebAuthToken)
    } catch { return $false }
}
...
function Get-FormValue {
    ...
    foreach ($pair in $Body -split '&') {
        $kv = $pair -split '=', 2
        if ($kv.Count -eq 2 -and [System.Net.WebUtility]::UrlDecode($kv[0]) -eq $Name) {
            return [System.Net.WebUtility]::UrlDecode($kv[1].Replace('+',' '))
        }
    }
    return ''
}
...
function Get-WebLoginPage {
    ...
    if (-not [string]::IsNullOrWhiteSpace($Message)) {
        ...
        $msgHtml = "<div class='msg'>$safeMsg</div>"
    }
    return @"
<!DOCTYPE html>
<html>
...
<style>
*{box-sizing:border-box}body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#1a1d21;color:#fff;font-family:'Segoe UI',Arial,sans-serif}.card{width:390px;max-width:92vw;background:#252a33;border:1px solid #3a4150;border-radius:14px;padding:28px;box-shadow:0 18px 50px rgba(0,0,0,.35)}h1{font-size:22px;margin:0 0 8px;text-align:center;color:#dce7f7}.sub{text-align:center;color:#9aa6b5;margin-bottom:22px;font-size:13px}.field{margin-bottom:14px}label{display:block;font-size:12px;color:#aeb8c8;margin-bottom:6px;font-weight:600}input{width:100%;padding:12px;border:1px solid #455063;border-radius:8px;background:#15191f;color:#fff;font-size:14px;outline:none}input:focus{border-color:#4da3ff;box-shadow:0 0 0 3px rgba(77,163,255,.15)}button{width:100%;padding:12px;border:0;border-radius:8px;background:#2ecc71;color:#fff;font-weight:700;cursor:pointer;font-size:14px;margin-top:6px}button:hover{filter:brightness(1.08)}.msg{background:#4a2424;border:1px solid #e45b5b;color:#ffd7d7;padding:10px;border-radius:8px;margin-bottom:14px;text-align:center;font-size:13px}.hint{font-size:11px;color:#7f8a9b;text-align:center;margin-top:16px}
</style>
</head>
<body>
<form class="card" method="post" action="/login" autocomplete="off">
<h1>PC Audit Dashboard</h1>
<div class="sub">Login required to access the webpage URL</div>
$msgHtml
<div class="field"><label>Username</label><input name="username" autofocus required></div>
<div class="field"><label>Password</label><input name="password" type="password" required></div>
<button type="submit">LOGIN</button>
<div class="hint">Protected dashboard session • Cache disabled</div>
</form>
</body>
</html>
"@
...
function Process-WebLoginRequest {
    param($Context)
    try {
        $reader = New-Object System.IO.StreamReader($Context.Request.InputStream, [System.Text.Encoding]::UTF8)
        $body = $reader.ReadToEnd()
        $reader.Close()
        $u = Get-FormValue -Body $body -Name 'username'
        $p = Get-FormValue -Body $body -Name 'password'
        if ($u -eq $script:WebAuthUsername -and $p -eq $script:WebAuthPassword) {
            $cookie = New-Object System.Net.Cookie($script:WebAuthCookieName, $script:WebAuthToken, '/')
            ...
            Set-NoCacheHeaders -Context $Context
            $Context.Response.OutputStream.Close()
            Write-Log -Message "Web login successful for $u" -Level 'INFO'
            return
        }
        Write-Log -Message "Web login failed for $u" -Level 'WARN'
        Send-HttpTextResponse -Context $Context -Html (Get-WebLoginPage -Message 'Invalid username or password') -StatusCode 401
    } catch {
        Write-Log -Message "Web login error: $_" -Level 'ERROR'
        Send-HttpTextResponse -Context $Context -Html (Get-WebLoginPage -Message 'Login error. Please try again.') -StatusCode 500
    }
}
...
function Get-QueryValue {
    ...
    foreach ($pair in $q -split '&') {
        $kv = $pair -split '=', 2
        if ($kv.Count -ge 1 -and [System.Net.WebUtility]::UrlDecode($kv[0]) -eq $Name) {
            if ($kv.Count -eq 2) { return [System.Net.WebUtility]::UrlDecode($kv[1].Replace('+',' ')) }
        }
    }
    return ''
}
...
#   netsh http add urlacl url=http://10.209.110.111:8080/ user=YOUR_USERNAME
#
# REQUEST HANDLING:
...
function Start-DashboardServer {
   ...
   $prefix = "http://10.209.110.111:$Port/"
   ...
   try {
       ...
       # EXE FIX COMMENTED: Console Write-Host can become popup in converted EXE.
       # Write-Host "netsh http add urlacl url=http://10.209.110.111:$Port user=$env:USERNAME" -ForegroundColor Cyan
       throw
   }
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           # WEB LOGIN: allow login page/post first, then protect all dashboard pages.
           if ($path -eq '/login' -and $context.Request.HttpMethod -eq 'POST') {
               Process-WebLoginRequest -Context $context
               continue
           }
           elseif ($path -eq '/login') {
               Send-HttpTextResponse -Context $context -Html (Get-WebLoginPage)
               continue
           }

           if (-not (Test-WebAuthenticated -Context $context)) {
               Send-HttpTextResponse -Context $context -Html (Get-WebLoginPage) -StatusCode 401
               continue
           }
           ...
           elseif ($path -match '^/details') {
               ...
               $type = Get-QueryValue -Query $query -Name 'type'
           ...
           # --- SEND RESPONSE ---
           Send-HttpTextResponse -Context $context -Html $responseHtml
...
function Show-MainGui {
    ...
    $script:GuiLastActivity = Get-Date
    $script:GuiAutoLockActive = $false
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    # ================================================================================================
    # AUTO-LOCK - LOCK CONTROLLER AFTER 5 MINUTES OF INACTIVITY
    # ================================================================================================
    function Update-GuiActivity {
        if (-not $script:GuiAutoLockActive) { $script:GuiLastActivity = Get-Date }
    }

    function Register-GuiActivityEvents {
        param([System.Windows.Forms.Control]$Control)
        try {
            $Control.Add_MouseMove({ Update-GuiActivity })
            $Control.Add_MouseDown({ Update-GuiActivity })
            $Control.Add_KeyDown({ Update-GuiActivity })
            $Control.Add_Click({ Update-GuiActivity })
            foreach ($child in $Control.Controls) { Register-GuiActivityEvents -Control $child }
        } catch { }
    }

    function Lock-GuiController {
        if ($script:GuiAutoLockActive) { return }
        $script:GuiAutoLockActive = $true
        Add-GuiLog "Controller auto-locked after 5 minutes of inactivity. Server keeps running." "WARN"
        try { $form.Hide() } catch { }
        $ok = Show-LoginWindow
        if ($ok) {
            $script:GuiLastActivity = Get-Date
            ...
            Add-GuiLog "Controller unlocked successfully." "INFO"
        } else {
            $script:GuiAutoLockActive = $false
            $script:GuiLastActivity = Get-Date
            Add-GuiLog "Controller remains minimized/hidden after auto-lock login was cancelled." "WARN"
        }
    }
    ...
    Register-GuiActivityEvents -Control $form
    ...
    $timer.Add_Tick({
        ...
        if (-not $script:GuiAutoLockActive -and $script:GuiLastActivity -and (((Get-Date) - $script:GuiLastActivity).TotalMinutes -ge 5)) {
            Lock-GuiController
        }
