...
function Get-DetailsPage {
   ...
   if ($isPeriodType) {
       ...
       $hdr = ($columns | ForEach-Object { "<th>$(Get-HtmlEncoded $_)</th>" }) -join ""
...
.filterbar{display:flex;gap:10px;align-items:center;margin:8px 0 14px;flex-wrap:wrap}.searchbox{width:320px;max-width:100%;padding:10px;border-radius:7px;border:1px solid #4a5362;background:#151a21;color:#fff}.clearfilters{padding:10px 14px;border:0;border-radius:7px;background:#596273;color:#fff;cursor:pointer}.filter-btn{float:right;margin-left:6px;background:#586273;color:#fff;border:0;border-radius:4px;width:22px;height:20px;cursor:pointer;font-size:11px}.filter-menu{position:fixed;z-index:9999;background:#252b34;border:1px solid #4b5566;border-radius:8px;padding:10px;width:260px;max-height:360px;box-shadow:0 12px 35px rgba(0,0,0,.45)}.filter-menu input[type=text]{width:100%;padding:8px;border-radius:6px;border:1px solid #4b5566;background:#151a21;color:#fff;margin-bottom:8px}.filter-list{max-height:210px;overflow:auto;border-top:1px solid #3e4654;border-bottom:1px solid #3e4654;padding:6px 0}.filter-item{display:block;padding:4px 2px;color:#e6edf7;font-size:12px}.filter-actions{display:flex;gap:8px;margin-top:8px}.filter-actions button{flex:1;padding:8px;border:0;border-radius:6px;cursor:pointer}.filter-ok{background:#2ecc71;color:#fff}.filter-clear{background:#e67e22;color:#fff}.filter-active{background:#2e8af7!important}
...
<div class="filterbar"><input id="globalSearch" class="searchbox" placeholder="Keyword search..." oninput="applyAllFilters()"><button class="clearfilters" onclick="clearAllFilters()">Clear Filters</button></div>
...
function normalizeDateOnly(v){ let m=(v||'').match(/\d{4}-\d{2}-\d{2}|\d{1,2}[\/\-]\d{1,2}[\/\-]\d{4}/); return m?m[0]:(v||'').trim(); }
function getCellValue(row, idx){ let cell=row.cells[idx]; if(!cell) return ''; let th=document.querySelectorAll('#t thead th')[idx]; let name=(th?th.childNodes[0].textContent:'').toLowerCase(); let txt=cell.innerText.trim(); if(name.includes('date') || name.includes('time')) return normalizeDateOnly(txt); return txt; }
function setupExcelFilters(){
  document.querySelectorAll('#t thead th').forEach(function(th,idx){
    let b=document.createElement('button'); b.className='filter-btn'; b.innerHTML='▼'; b.title='Filter'; b.onclick=function(e){e.stopPropagation(); openFilterMenu(idx,b);}; th.appendChild(b);
  });
}
function openFilterMenu(idx,btn){
  closeFilterMenu(); currentFilterCol=idx;
  let rows=Array.from(document.querySelectorAll('#t tbody tr'));
  let vals=[...new Set(rows.map(r=>getCellValue(r,idx)).filter(v=>v!==''))].sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
  let selected=activeFilters[idx] || new Set(vals);
  let menu=document.createElement('div'); menu.className='filter-menu'; menu.id='filterMenu';
  menu.innerHTML='<input type="text" id="filterSearch" placeholder="Search in this column..."><div><label class="filter-item"><input type="checkbox" id="selectAll"> Select All</label></div><div class="filter-list" id="filterList"></div><div class="filter-actions"><button class="filter-ok" id="filterOk">OK</button><button class="filter-clear" id="filterClear">Clear</button></div>';
  document.body.appendChild(menu); let r=btn.getBoundingClientRect(); menu.style.left=Math.min(r.left,window.innerWidth-270)+'px'; menu.style.top=(r.bottom+4)+'px';
  function render(){ let q=document.getElementById('filterSearch').value.toLowerCase(); let list=document.getElementById('filterList'); list.innerHTML=''; vals.filter(v=>v.toLowerCase().includes(q)).slice(0,700).forEach(function(v){ let id='fv_'+Math.random().toString(36).slice(2); let checked=selected.has(v)?'checked':''; list.insertAdjacentHTML('beforeend','<label class="filter-item"><input type="checkbox" value="'+escapeHtml(v)+'" '+checked+'> '+escapeHtml(v)+'</label>'); }); document.getElementById('selectAll').checked = vals.length>0 && vals.every(v=>selected.has(v)); }
  document.getElementById('filterSearch').oninput=render;
  document.getElementById('selectAll').onchange=function(){ selected=this.checked?new Set(vals):new Set(); render(); };
  document.getElementById('filterList').onchange=function(e){ if(e.target.type==='checkbox'){ if(e.target.checked) selected.add(e.target.value); else selected.delete(e.target.value); } };
  document.getElementById('filterOk').onclick=function(){ activeFilters[idx]=selected; updateFilterButtons(); applyAllFilters(); closeFilterMenu(); };
  document.getElementById('filterClear').onclick=function(){ delete activeFilters[idx]; updateFilterButtons(); applyAllFilters(); closeFilterMenu(); };
  render();
}
function escapeHtml(s){return (s||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
function closeFilterMenu(){ let m=document.getElementById('filterMenu'); if(m) m.remove(); }
function updateFilterButtons(){ document.querySelectorAll('.filter-btn').forEach((b,i)=>b.classList.toggle('filter-active',!!activeFilters[i])); }
function applyAllFilters(){
 ...
 document.querySelectorAll('#t tbody tr').forEach(function(row){
   let ok=true; if(q && !row.innerText.toLowerCase().includes(q)) ok=false;
   Object.keys(activeFilters).forEach(function(k){ if(!activeFilters[k].has(getCellValue(row,parseInt(k)))) ok=false; });
   row.style.display=ok?'':'none';
 });
}
function clearAllFilters(){ activeFilters={}; let s=document.getElementById('globalSearch'); if(s) s.value=''; updateFilterButtons(); applyAllFilters(); }
document.addEventListener('click',function(e){ if(!e.target.closest('.filter-menu') && !e.target.closest('.filter-btn')) closeFilterMenu(); });
document.addEventListener('DOMContentLoaded',setupExcelFilters);
...
"@
   ...
   elseif ($isIssueType) {
       ...
       $hdr = "<th>SNo</th><th>Computer Name</th><th>IP Address</th><th>Network</th><th>$statusColumnName</th><th>Auditor ID</th><th>Audit Date</th>"
...
function Start-DashboardServer {
   ...
   if (-not $script:GuiSettings -or [bool]$script:GuiSettings.AutoOpenBrowser) { Start-Process (Get-GuiDashboardUrl) -ErrorAction SilentlyContinue }
   ...
   while ($script:RunServer -and $script:Listener.IsListening) {
       try {
           ...
           # WEB AUTH ROUTES - login/logout/auth-check
           if ($path -eq '/login') {
               if ($context.Request.HttpMethod -eq 'POST') {
                   $form = Read-RequestForm -Request $context.Request
                   $u = [string]$form['username']
                   $p = [string]$form['password']
                   if ($u -eq [string]$script:GuiSettings.WebUsername -and (Get-Sha256Text $p) -eq [string]$script:GuiSettings.WebPasswordHash) {
                       $token = New-WebSessionToken
                       $script:WebSessions[$token] = [pscustomobject]@{ Version = [string]$script:GuiSettings.WebCredentialVersion; Expires = (Get-Date).AddHours(8) }
                       $context.Response.Headers.Add('Set-Cookie', "PCAuditAuth=$token; Path=/; HttpOnly; SameSite=Lax")
                       Send-Redirect -Context $context -Location '/'
                       continue
                   } else {
                       Send-TextResponse -Context $context -Text (Get-WebLoginPage "Invalid username or password") -StatusCode 401
                       continue
                   }
               } else {
                   $msg = if ($query -match 'expired=1') { 'Session expired. Please login again.' } else { '' }
                   Send-TextResponse -Context $context -Text (Get-WebLoginPage $msg) -StatusCode 401
                   continue
               }
           }
           elseif ($path -eq '/logout') {
               $token = Get-RequestCookieValue -Request $context.Request -Name 'PCAuditAuth'
               if ($token -and $script:WebSessions.ContainsKey($token)) { $script:WebSessions.Remove($token) }
               $context.Response.Headers.Add('Set-Cookie', 'PCAuditAuth=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; SameSite=Lax')
               Send-Redirect -Context $context -Location '/login'
               continue
           }
           elseif ($path -eq '/auth-check') {
               if (Test-WebAuthenticated -Request $context.Request) { Send-TextResponse -Context $context -Text '{"ok":true}' -ContentType 'application/json; charset=utf-8' -StatusCode 200 }
               else { Send-TextResponse -Context $context -Text '{"ok":false}' -ContentType 'application/json; charset=utf-8' -StatusCode 401 }
               continue
           }
           elseif (-not (Test-WebAuthenticated -Request $context.Request)) {
               Send-TextResponse -Context $context -Text (Get-WebLoginPage "Please login to access dashboard") -StatusCode 401
               continue
           }
           ...
           # --- SEND RESPONSE ---
           $responseHtml = Add-AuthCheckScript $responseHtml
...
# ====================================================================================================
# WEB LOGIN / SESSION SECURITY - ADDED
# ====================================================================================================
# Default web credential: PCAudit / PC_Audit
# Password is stored as SHA256 hash in pc_audit_gui_settings.json.
# When web password is changed, WebCredentialVersion changes and all old browser sessions become invalid.
# ====================================================================================================
$script:WebSessions = @{}
...
function Get-DefaultWebPasswordHash { return Get-Sha256Text "PC_Audit" }

function New-WebSessionToken {
...
function Get-RequestCookieValue {
    ...
    try {
        ...
        foreach ($part in $cookieHeader -split ';') {
            ...
            if ($kv.Count -eq 2 -and $kv[0] -eq $Name) { return [System.Web.HttpUtility]::UrlDecode($kv[1]) }
        }
    } catch { }
    return $null
}

function Test-WebAuthenticated {
    param($Request)
    try {
        $token = Get-RequestCookieValue -Request $Request -Name 'PCAuditAuth'
        if ([string]::IsNullOrWhiteSpace($token)) { return $false }
        if (-not $script:WebSessions.ContainsKey($token)) { return $false }
        $session = $script:WebSessions[$token]
        if ($session.Version -ne [string]$script:GuiSettings.WebCredentialVersion) { return $false }
        if ($session.Expires -lt (Get-Date)) { $script:WebSessions.Remove($token); return $false }
        $session.Expires = (Get-Date).AddHours(8)
        return $true
    } catch { return $false }
}

function Read-RequestForm {
    param($Request)
    $result = @{}
    try {
...
function Get-WebLoginPage {
    param([string]$Message = "")
    $safeMsg = Get-HtmlEncoded $Message
    return @"
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>PC Audit Dashboard Login</title>
<style>
*{box-sizing:border-box}body{margin:0;font-family:'Segoe UI',Arial,sans-serif;background:#161a20;color:#fff;min-height:100vh;display:flex;align-items:center;justify-content:center}.card{width:390px;background:#252b34;border:1px solid #384150;border-radius:14px;padding:28px;box-shadow:0 18px 45px rgba(0,0,0,.35)}h1{font-size:22px;margin:0 0 6px;text-align:center}.sub{color:#9aa6b8;text-align:center;margin-bottom:22px;font-size:13px}label{display:block;color:#cbd4e1;font-size:13px;margin:12px 0 6px}input{width:100%;padding:12px;border-radius:8px;border:1px solid #465164;background:#151a21;color:#fff;font-size:14px}button{width:100%;padding:12px;margin-top:18px;border:0;border-radius:8px;background:#2e8af7;color:white;font-weight:700;cursor:pointer}.msg{background:#4b1f24;color:#ffb8c0;border:1px solid #80323d;border-radius:8px;padding:10px;margin-bottom:14px;text-align:center;font-size:13px}.note{font-size:12px;color:#7f8da3;text-align:center;margin-top:14px}
</style>
</head>
<body>
<form class="card" method="post" action="/login">
<h1>PC Audit Dashboard</h1>
<div class="sub">Enter web credential to continue</div>
$(if($safeMsg){"<div class='msg'>$safeMsg</div>"})
<label>Username</label><input name="username" autocomplete="username" autofocus>
<label>Password</label><input name="password" type="password" autocomplete="current-password">
<button type="submit">LOGIN</button>
<div class="note">401 authentication enabled for direct URL access</div>
</form>
</body>
</html>
"@
...
function Add-AuthCheckScript {
    param([string]$Html)
    $js = @"
<script>
(function(){
  function checkAuth(){
    fetch('/auth-check',{cache:'no-store'}).then(function(r){ if(r.status===401){ location.href='/login?expired=1'; } }).catch(function(){});
  }
  setInterval(checkAuth,15000);
})();
</script>
"@
    if ($Html -match '</body>') { return ($Html -replace '</body>', ($js + '</body>')) }
    return $Html + $js
}
...
function Initialize-GuiSettings {
    $defaults = [ordered]@{
        ...
        AutoRefreshLog = $true
        WebUsername = "PCAudit"
        WebPasswordHash = (Get-DefaultWebPasswordHash)
        WebCredentialVersion = ([guid]::NewGuid().ToString())
    }
...
function Apply-GuiSettings {
    ...
    if ([string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebUsername)) { $script:GuiSettings.WebUsername = "PCAudit" }
    if ([string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebPasswordHash)) { $script:GuiSettings.WebPasswordHash = Get-DefaultWebPasswordHash }
    if ([string]::IsNullOrWhiteSpace([string]$script:GuiSettings.WebCredentialVersion)) { $script:GuiSettings.WebCredentialVersion = [guid]::NewGuid().ToString() }
}
...
function Show-SettingsWindow {
    ...
    $f.Size = New-Object System.Drawing.Size(660, 590)
    ...
    New-Label "Web Username" 24 256 | Out-Null
    $txtWebUser = New-Text ([string]$script:GuiSettings.WebUsername) 190 254 220
    New-Label "Old Web Password" 24 294 | Out-Null
    $txtOldWebPass = New-Text "" 190 292 220
    $txtOldWebPass.UseSystemPasswordChar = $true
    New-Label "New Web Password" 24 332 | Out-Null
    $txtNewWebPass = New-Text "" 190 330 220
    $txtNewWebPass.UseSystemPasswordChar = $true
    New-Label "Confirm Password" 24 370 | Out-Null
    $txtConfirmWebPass = New-Text "" 190 368 220
    $txtConfirmWebPass.UseSystemPasswordChar = $true
    ...
    $chkBrowser.Location=New-Object System.Drawing.Point(190,410); $chkBrowser.Size=New-Object System.Drawing.Size(260,24)
    ...
    $chkTray.Location=New-Object System.Drawing.Point(190,438); $chkTray.Size=New-Object System.Drawing.Size(300,24)
    $chkTray.ForeColor=[System.Drawing.Color]::White; $chkTray.BackColor=$f.BackColor
    $f.Controls.Add($chkTray)

    $btnSave = New-Object System.Windows.Forms.Button
    $btnSave.Text="SAVE"; $btnSave.Location=New-Object System.Drawing.Point(374,500); $btnSave.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnCancel.Text="CANCEL"; $btnCancel.Location=New-Object System.Drawing.Point(486,500); $btnCancel.Size=New-Object System.Drawing.Size(95,34)
    ...
    $btnSave.Add_Click({
        ...
        $webUser = $txtWebUser.Text.Trim()
        if ([string]::IsNullOrWhiteSpace($webUser)) {
            [System.Windows.Forms.MessageBox]::Show("Web username cannot be empty.", "Settings", "OK", "Warning") | Out-Null
            return
        }
        $newPass = $txtNewWebPass.Text
        $confirmPass = $txtConfirmWebPass.Text
        if (-not [string]::IsNullOrWhiteSpace($newPass) -or -not [string]::IsNullOrWhiteSpace($confirmPass)) {
            if ((Get-Sha256Text $txtOldWebPass.Text) -ne [string]$script:GuiSettings.WebPasswordHash) {
                [System.Windows.Forms.MessageBox]::Show("Old web password is wrong. Password not changed.", "Settings", "OK", "Warning") | Out-Null
                return
            }
            if ($newPass -ne $confirmPass) {
                [System.Windows.Forms.MessageBox]::Show("New password and confirm password do not match.", "Settings", "OK", "Warning") | Out-Null
                return
            }
            if ($newPass.Length -lt 4) {
                [System.Windows.Forms.MessageBox]::Show("New password must be at least 4 characters.", "Settings", "OK", "Warning") | Out-Null
                return
            }
            $script:GuiSettings.WebPasswordHash = Get-Sha256Text $newPass
            $script:GuiSettings.WebCredentialVersion = [guid]::NewGuid().ToString()
            $script:WebSessions.Clear()
            Add-GuiLog "Web credential password changed. Existing dashboard sessions invalidated." "INFO"
        }
        $script:GuiSettings.WebUsername = $webUser
...
$form.Add_MouseUp({ $script:ResizeDragActive = $false })
    ...
    $script:BtnStart.Add_Click({ Start-GuiDashboard })
    $script:BtnStop.Add_Click({ Stop-GuiDashboard })
    $script:BtnRestart.Add_Click({ Restart-GuiDashboard })
    ...
    # AUTO-LOCK: hide/lock controller after 5 minutes inactivity while server is running
    $script:LastGuiActivity = Get-Date
    $resetActivity = { $script:LastGuiActivity = Get-Date }
    $form.Add_MouseMove($resetActivity); $form.Add_KeyDown($resetActivity); $form.Add_Click($resetActivity)
    foreach($ctl in $form.Controls){ try { $ctl.Add_MouseMove($resetActivity); $ctl.Add_Click($resetActivity); $ctl.Add_KeyDown($resetActivity) } catch { } }
    $autoLockTimer = New-Object System.Windows.Forms.Timer
    $autoLockTimer.Interval = 30000
    $autoLockTimer.Add_Tick({
        if (Is-DashboardRunning -and ((Get-Date) - $script:LastGuiActivity).TotalMinutes -ge 5) {
            $autoLockTimer.Stop()
            $form.Hide()
            Add-GuiLog "Controller auto-locked after 5 minutes inactivity." "INFO"
